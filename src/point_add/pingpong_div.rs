...
fn interleaved_odd_loan_enabled() -> bool {
    std::env::var("SUB4_PP_INTERLEAVED_ODD_LOAN")
        .ok()
        .as_deref()
        == Some("1")
}
...
fn loan_interleaved_odd_passengers(
    ...
    if !interleaved_odd_loan_enabled() {
        return None;
    }
...
fn restore_interleaved_odd_passengers(b: &mut B, passengers: Option<[QubitId; 2]>) {
