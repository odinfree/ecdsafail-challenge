...
pub(crate) fn pingpong_mod_mul_div_in_place(
    ...
    let _ = generic_s2_elision_enabled();
        ...
        (PingPongDirection::Multiply, Some(plan)) => {
            ...
            for r in (0..plan.r1.min(rounds)).rev() {
                ...
                if r == 2 && generic_s2_elision_enabled() {
                    ...
                    walk_back_round2_with_s1_host(b, &mut u, &mut v, tape[1], rounds);
                } else {
                    walk_back_round(b, &mut u, &mut v, r, sign, rounds);
                }
            }
...
/// Exact `0` preserves the retained-s2 Q1264 control; exact `1` selects the
/// source-baked Q1263 composition. Unset and all other values fail closed.
fn generic_s2_elision_enabled() -> bool {
    let enabled = match std::env::var_os("SUB4_PP_ELIDE_GENERIC_S2") {
        None => false,
        Some(value) if value == "0" => false,
        Some(value) if value == "1" => true,
        Some(value) => panic!(
    ...
    if enabled {
        require_round0_a0_exact_route();
        assert_eq!(
            std::env::var("SUB4_PP_ELIDE_GENERIC_S2").ok().as_deref(),
            Some("1"),
            "SUB4_PP_ELIDE_GENERIC_S2 requires its raw feature value to be 1"
        );
    }
    enabled
}
...
fn walk_round2_with_s1_host(
    ...
    match walk_low_chunk(round, width) {
        Some(low) => signed_add_wrapping_sigma_split(
            b, host, source, target, true, low, top_skip,
        ),
...
fn walk_back_round2_with_s1_host(
    ...
    match walk_low_chunk(round, width) {
        Some(low) => signed_add_wrapping_sigma_split(
            b, host, source, target, false, low, top_skip,
        ),
...
pub(crate) fn round0_a0_elision_selfcheck() {
    ...
    // Isolate the a0 differential from the composed generic-s2 candidate:
    // the retained a0 control is intentionally outside the hosted-s2 route.
    std::env::set_var("SUB4_PP_ELIDE_GENERIC_S2", "0");
