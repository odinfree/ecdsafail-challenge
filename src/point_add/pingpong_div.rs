...
/// alternative width tables (e.g. the census greedy frontier) on this base.
...
/// (identity index) and rescaled (`round*703/697`) traversals, through the
/// real `value_width` code path.  Gated by `SUB4_DUMP_WSCHED` in
/// `build_circuit`, so it never runs in the shipped stream.
pub(crate) fn dump_width_schedule() {
    std::env::remove_var("SUB4_PP_WIDTH_RESCALE");
    let base: Vec<usize> = (0..700).map(value_width).collect();
    std::env::set_var("SUB4_PP_WIDTH_RESCALE", "1");
