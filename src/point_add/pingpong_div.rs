...
pub(crate) fn pingpong_mod_mul_div_in_place(
    ...
    // Teddy Pender's Burn-the-House-Down production falsifier deliberately
    // enters through the real divide component ABI.  The gated slice is an
    // identity: it walks and replays rounds 0..=3, reverses both halves, and
    // returns the caller's registers untouched.  Normal submission builds do
    // not set this variable and therefore take the byte-for-byte old path.
    if direction == PingPongDirection::Divide
        && std::env::var_os("SUB4_PP_RETAINED_PRODUCTION_PREFIX_SLICE").is_some()
    {
        retained_denominator_production_prefix_slice(b, denominator, numerator);
        return;
    }
...
/// Exact inverse of `fused_lift_round0_forward` for Teddy Pender's bounded
/// retained-word diagnostic. Production still selects its existing sparse
/// endpoint through `fused_lift_round0_reverse`; this duplicate full endpoint
/// is reachable only from the environment-gated self-check below.
fn teddy_fused_lift_round0_reverse_full(b: &mut B, v: &[QubitId], a0: QubitId) {
    debug_assert_eq!(v.len(), VALUE_WIDTH);
    let not_a1 = b.alloc_qubit();
    b.cx(v[VALUE_WIDTH - 1], not_a1);
    let f = U256::MAX
...
const RETAINED_PRODUCTION_PREFIX_ROUNDS: usize = 4;

/// Build-time and simulator checkpoints for Teddy Pender's first low-five
/// slice inside the production divide ABI.  Every listed qubit is temporary;
/// the emitter returns with only `denominator` and `numerator` active.
struct RetainedProductionPrefixSliceReceipt {
    forward_sign_checkpoints: [usize; RETAINED_PRODUCTION_PREFIX_ROUNDS],
    forward_signs: [QubitId; RETAINED_PRODUCTION_PREFIX_ROUNDS],
    replay_round_checkpoints: [usize; RETAINED_PRODUCTION_PREFIX_ROUNDS],
    normalization_checkpoints: [usize; 2],
    replay_midpoint: usize,
    replay_reverse_checkpoint: usize,
    retained: Vec<QubitId>,
    normalization_flag: QubitId,
    replay_sign: QubitId,
    coefficient: Vec<QubitId>,
    forward_persistent_active: [u32; RETAINED_PRODUCTION_PREFIX_ROUNDS],
    replay_persistent_active: [u32; RETAINED_PRODUCTION_PREFIX_ROUNDS],
}

struct ProductionPrefixReferenceReceipt {
    replay_midpoint: usize,
    replay_reverse_checkpoint: usize,
    coefficient: Vec<QubitId>,
}

/// Toggle the exact production walk sign into `sign` from Teddy Pender's
/// five-bit sufficient statistic. Round zero remains on the one-wire tape.
/// Applying this twice is the identity.
fn retained_denominator_prefix_sign_oracle(
    b: &mut B,
    retained: &[QubitId],
    round: usize,
    sign: QubitId,
    ...
    b.x(sign);
    match round {
        1 => b.cx(retained[2], sign),
        2 => b.cx(retained[3], sign),
        3 => {
            b.cx(retained[0], sign);
            b.cx(retained[2], sign);
            b.cx(retained[3], sign);
            b.cx(retained[4], sign);
        }
        _ => panic!("bounded retained production prefix reconstructs rounds 1 through 3"),
    }
}

fn retained_denominator_prefix_toggle_round1_sentinel(
    b: &mut B,
    retained: &[QubitId],
    normalization_flag: QubitId,
    x: &[QubitId],
    ...
    if std::env::var_os(
        "SUB4_PP_RETAINED_PRODUCTION_PREFIX_PRODUCTION_FAITHFUL",
    )
    .is_some()
    {
        return;
    }
    retained_denominator_prefix_sign_oracle(b, retained, 1, normalization_flag);
    for i in 0..N {
        if SECP256K1_P.bit(i) {
            b.cx(normalization_flag, x[i]);
        }
    }
    retained_denominator_prefix_sign_oracle(b, retained, 1, normalization_flag);
}

fn finish_bounded_production_walk(
    b: &mut B,
    u: &mut [QubitId],
    v: &mut [QubitId],
    wanted_u: &[QubitId],
    ...
    restore_wire_layout(b, u, v, wanted_u, wanted_v);
    b.free_vec(&v[N..]);
    for i in 0..N {
        if SECP256K1_P.bit(i) {
            b.x(u[i]);
...
/// Teddy Pender's exact next Burn-the-House-Down falsifier, embedded behind an
/// environment gate in [`pingpong_mod_mul_div_in_place`].
///
/// The slice keeps denominator bits zero through four, generates the real walk
/// prefix, keeps round zero on the tape, and clears generated signs one through
/// three immediately with Teddy's low-five oracle. It then replays rounds
/// 0..=3 with one shared sign and one atomic normalization flag, reverses that
/// replay, reconstructs each sign just in time to reverse the walk, and returns
/// every added wire through the public component ABI.
fn retained_denominator_production_prefix_slice(
    b: &mut B,
    denominator: &[QubitId],
    numerator: &[QubitId],
) -> RetainedProductionPrefixSliceReceipt {
    assert_eq!(denominator.len(), N);
    assert_eq!(numerator.len(), N);
    assert!(
        fused_lift_round0_enabled(),
        "Teddy's retained prefix is bound to the production fused round-zero lift"
    );
    let entry_active = b.active_qubits;

    b.set_phase("teddy_retained_prod_prefix_retain");
    let retained = b.alloc_qubits(5);
    for i in 0..retained.len() {
        b.cx(denominator[i], retained[i]);
    }
    let normalization_flag = b.alloc_qubit();
    ...
    b.set_phase("teddy_retained_prod_prefix_walk");
    let mut forward_sign_checkpoints = [0usize; RETAINED_PRODUCTION_PREFIX_ROUNDS];
    let mut forward_signs = [QubitId(0); RETAINED_PRODUCTION_PREFIX_ROUNDS];
    let mut forward_persistent_active = [0u32; RETAINED_PRODUCTION_PREFIX_ROUNDS];
    for round in 0..RETAINED_PRODUCTION_PREFIX_ROUNDS {
        let generated_sign = walk_round(b, &mut u, &mut v, round);
        forward_signs[round] = generated_sign;
        if round > 0 {
            retained_denominator_prefix_sign_oracle(
                b,
                &retained,
                round,
                generated_sign,
        ...
        forward_sign_checkpoints[round] = b.ops.len();
        if round > 0 {
            b.free(generated_sign);
        }
        forward_persistent_active[round] = b.active_qubits;
    }
    assert!(
        forward_persistent_active
            .windows(2)
            .all(|pair| pair[1] <= pair[0]),
        "retained production walk state grew with the round"
    );
    b.set_phase("teddy_retained_prod_prefix_replay");
    let coefficient = b.alloc_qubits(N);
    let replay_sign = b.alloc_qubit();
    let mut replay_round_checkpoints = [0usize; RETAINED_PRODUCTION_PREFIX_ROUNDS];
    let mut replay_persistent_active = [0u32; RETAINED_PRODUCTION_PREFIX_ROUNDS];
    let round_zero = forward_signs[0];

    replay_halving_round(b, 0, round_zero, &coefficient, numerator);
    replay_round_checkpoints[0] = b.ops.len();
    replay_persistent_active[0] = b.active_qubits;

    retained_denominator_prefix_sign_oracle(b, &retained, 1, replay_sign);
    replay_halving_round(b, 1, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 1, replay_sign);
    replay_round_checkpoints[1] = b.ops.len();
    replay_persistent_active[1] = b.active_qubits;

    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
        normalization_flag,
        &coefficient,
    );
    let mut normalization_checkpoints = [0usize; 2];
    normalization_checkpoints[0] = b.ops.len();
    retained_denominator_prefix_sign_oracle(b, &retained, 2, replay_sign);
    replay_halving_round(b, 2, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 2, replay_sign);
    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
    ...
    replay_round_checkpoints[2] = b.ops.len();
    replay_persistent_active[2] = b.active_qubits;
    ...
    normalization_checkpoints[1] = b.ops.len();
    retained_denominator_prefix_sign_oracle(b, &retained, 3, replay_sign);
    replay_halving_round(b, 3, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 3, replay_sign);
    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
    ...
    replay_round_checkpoints[3] = b.ops.len();
    replay_persistent_active[3] = b.active_qubits;
    assert!(
        replay_persistent_active
            .windows(2)
            .all(|pair| pair[1] == pair[0]),
        "retained replay state grew with the round"
    );
    let replay_midpoint = b.ops.len();

    // Reverse the exact bounded replay.  Each normalization call is atomic:
    // the sole flag is zero again before a replay sign is reconstructed.
    b.set_phase("teddy_retained_prod_prefix_replay_reverse");
    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
        normalization_flag,
    ...
    retained_denominator_prefix_sign_oracle(b, &retained, 3, replay_sign);
    replay_doubling_round(b, 3, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 3, replay_sign);
    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
    ...
    retained_denominator_prefix_sign_oracle(b, &retained, 2, replay_sign);
    replay_doubling_round(b, 2, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 2, replay_sign);
    retained_denominator_prefix_toggle_round1_sentinel(
        b,
        &retained,
    ...
    retained_denominator_prefix_sign_oracle(b, &retained, 1, replay_sign);
    replay_doubling_round(b, 1, replay_sign, &coefficient, numerator);
    retained_denominator_prefix_sign_oracle(b, &retained, 1, replay_sign);
    replay_doubling_round(b, 0, round_zero, &coefficient, numerator);
    let replay_reverse_checkpoint = b.ops.len();
    b.free(replay_sign);
    b.free_vec(&coefficient);
    b.free(normalization_flag);

    b.set_phase("teddy_retained_prod_prefix_walk_reverse");
    for round in (1..RETAINED_PRODUCTION_PREFIX_ROUNDS).rev() {
        let reconstructed_sign = b.alloc_qubit();
        retained_denominator_prefix_sign_oracle(
            b,
            &retained,
            round,
        ...
        walk_back_round(b, &mut u, &mut v, round, reconstructed_sign);
    }
    grow_to(b, &mut u, &mut v, VALUE_WIDTH);
    teddy_fused_lift_round0_reverse_full(b, &v, round_zero);

    b.set_phase("teddy_retained_prod_prefix_return");
    finish_bounded_production_walk(b, &mut u, &mut v, &wanted_u, &wanted_v);
    for i in 0..retained.len() {
        b.cx(denominator[i], retained[i]);
    }
    b.free_vec(&retained);
    assert_eq!(
        b.active_qubits, entry_active,
        "Teddy retained production slice did not return through the component ABI"
    );

    RetainedProductionPrefixSliceReceipt {
        forward_sign_checkpoints,
        forward_signs,
        replay_round_checkpoints,
        normalization_checkpoints,
        replay_midpoint,
        replay_reverse_checkpoint,
        retained,
        normalization_flag,
        replay_sign,
        coefficient,
...
/// The unchanged four-round production walk/replay prefix and its exact
/// inverse. It is deliberately independent of Teddy's low-five slice so
/// the midpoint comparison can falsify the new representation boundary.
fn unchanged_production_prefix_reference(
    b: &mut B,
    denominator: &[QubitId],
    numerator: &[QubitId],
) -> ProductionPrefixReferenceReceipt {
    assert_eq!(denominator.len(), N);
    assert_eq!(numerator.len(), N);
    assert!(fused_lift_round0_enabled());
    let entry_active = b.active_qubits;
    ...
    b.set_phase("teddy_prod_prefix_reference_walk");
    let mut tape = Vec::with_capacity(RETAINED_PRODUCTION_PREFIX_ROUNDS);
    for round in 0..RETAINED_PRODUCTION_PREFIX_ROUNDS {
        tape.push(walk_round(b, &mut u, &mut v, round));
    }
    let coefficient = b.alloc_qubits(N);
    let normalization_flag = b.alloc_qubit();
    b.set_phase("teddy_prod_prefix_reference_replay");
    replay_halving_round(b, 0, tape[0], &coefficient, numerator);
    replay_halving_round(b, 1, tape[1], &coefficient, numerator);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    replay_halving_round(b, 2, tape[2], &coefficient, numerator);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    replay_halving_round(b, 3, tape[3], &coefficient, numerator);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    let replay_midpoint = b.ops.len();

    b.set_phase("teddy_prod_prefix_reference_replay_reverse");
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    replay_doubling_round(b, 3, tape[3], &coefficient, numerator);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    replay_doubling_round(b, 2, tape[2], &coefficient, numerator);
    teddy_production_toggle_round1_sentinel(b, tape[1], normalization_flag, &coefficient);
    replay_doubling_round(b, 1, tape[1], &coefficient, numerator);
    replay_doubling_round(b, 0, tape[0], &coefficient, numerator);
    let replay_reverse_checkpoint = b.ops.len();
    b.free(normalization_flag);
    b.free_vec(&coefficient);
    b.set_phase("teddy_prod_prefix_reference_walk_reverse");
    for round in (1..RETAINED_PRODUCTION_PREFIX_ROUNDS).rev() {
        let sign = tape.pop().expect("reference tape holds bounded prefix sign");
        walk_back_round(b, &mut u, &mut v, round, sign);
    }
    let round_zero = tape.pop().expect("reference tape holds round-zero sign");
    grow_to(b, &mut u, &mut v, VALUE_WIDTH);
    teddy_fused_lift_round0_reverse_full(b, &v, round_zero);
    finish_bounded_production_walk(b, &mut u, &mut v, &wanted_u, &wanted_v);
    assert!(tape.is_empty());
    assert_eq!(b.active_qubits, entry_active);
...
/// Strong bounded census for the production-ABI low-five slice. The
/// 4,096 denominators exhaust all low-nine-bit residues under the same eight
/// high prefixes that cover Teddy's dirty-bridge states.  Every denominator is
/// paired with the zero-seeded replay state used by the bounded semantic
/// saddle.  `SUB4_PP_RETAINED_PRODUCTION_PREFIX_LIVE_NUMERATOR_STRESS=1`
/// deliberately widens this to nonzero live numerators; that is a stress probe,
/// not part of the first production integration gate.
pub(crate) fn retained_denominator_production_prefix_selfcheck() {
    ...
    let candidate_receipt = retained_denominator_production_prefix_slice(
        &mut candidate,
    ...
    let reference_receipt = unchanged_production_prefix_reference(
        &mut reference,
    ...
    let candidate_retained_reg = to_register(&candidate_receipt.retained);
    let candidate_coefficient_reg = to_register(&candidate_receipt.coefficient);
    ...
    let reference_coefficient_reg = to_register(&reference_receipt.coefficient);

    let mut stored_round_zero_mismatch = 0u64;
    let mut forward_sign_mismatch = 0u64;
    let mut replay_sign_mismatch = 0u64;
    let mut normalization_flag_mismatch = 0u64;
    let mut retained_state_mismatch = 0u64;
    let mut replay_reverse_mismatch = 0u64;
    let mut paired_replay_reverse_mismatch = 0u64;
    let mut continuation_mismatch = 0u64;
    let mut continuation_coefficient_mismatch = 0u64;
    let mut continuation_numerator_mismatch = 0u64;
    let mut candidate_final_classical = 0u64;
    let mut reference_final_classical = 0u64;
    let mut paired_final_classical_mismatch = 0u64;
    let mut paired_final_phase_xor_lanes = 0u64;
    let mut candidate_phase_lanes = 0u64;
    let mut reference_phase_lanes = 0u64;
    let mut candidate_ancilla_wire_lanes = 0u64;
    let mut reference_ancilla_wire_lanes = 0u64;
    let mut candidate_executed_toffoli = 0u64;
    let mut reference_executed_toffoli = 0u64;
    let live_numerator_stress = std::env::var_os(
        "SUB4_PP_RETAINED_PRODUCTION_PREFIX_LIVE_NUMERATOR_STRESS",
    )
    .is_some();
    let seed_mode = if live_numerator_stress {
        "nonzero_live_stress"
    } else {
        "zero_semantic"
    };
    let first_batch_only = std::env::var_os(
        "SUB4_PP_RETAINED_PRODUCTION_PREFIX_FIRST_BATCH_ONLY",
    )
    .is_some();
    let case_count = if first_batch_only { 64usize } else { 4096usize };

    let mut bases = teddy_retained_round3_high_prefixes();
    if first_batch_only {
        bases.truncate(1);
    }
    for (prefix, base) in bases.into_iter().enumerate() {
        let residue_batches = if first_batch_only { 1 } else { 8 };
        for residue_batch in 0..residue_batches {
            ...
            let denominators: Vec<U256> = (0..64)
                ...
                .collect();
            let numerators: Vec<U256> = if live_numerator_stress {
                denominators
                    .iter()
                    .enumerate()
                    .map(|(shot, &denominator)| {
                        let global = batch * 64 + shot;
                        let candidate = match global % 16 {
                            0 => U256::from(1),
                            1 => SECP256K1_P.wrapping_sub(U256::from(1)),
                            _ => denominator
                                .mul_mod(U256::from((global % 251 + 2) as u64), SECP256K1_P)
            ...
            candidate_shake.update(b"Teddy Pender paired production-prefix replay");
            ...
            for shot in 0..64 {
                candidate_sim.set_register(
                    &candidate_denominator_reg,
                    denominators[shot],
                    shot,
                );
                candidate_sim.set_register(&candidate_numerator_reg, numerators[shot], shot);
            }
            let mut candidate_batch_phase_mask = 0u64;

            let mut candidate_cursor = 0usize;
            for round in 0..RETAINED_PRODUCTION_PREFIX_ROUNDS {
                let checkpoint = candidate_receipt.forward_sign_checkpoints[round];
                candidate_sim.apply_iter(candidate_ops[candidate_cursor..checkpoint].iter());
                if round == 0 {
                    let expected = denominators.iter().enumerate().fold(
                        0u64,
                        |mask, (shot, denominator)| {
                            mask | (u64::from(denominator.bit(0)) << shot)
                        },
                    );
                    stored_round_zero_mismatch += (candidate_sim
                        .qubit(candidate_receipt.forward_signs[round])
                        ^ expected)
                        .count_ones() as u64;
                } else {
                    forward_sign_mismatch += candidate_sim
                        .qubit(candidate_receipt.forward_signs[round])
                        .count_ones() as u64;
                }
                normalization_flag_mismatch += candidate_sim
                    .qubit(candidate_receipt.normalization_flag)
                    .count_ones() as u64;
                for shot in 0..64 {
                    if candidate_sim.get_register(&candidate_retained_reg, shot)
                        != denominators[shot] & U256::from(31)
                    {
                        retained_state_mismatch += 1;
                    }
                }
                candidate_batch_phase_mask |= candidate_sim.phase;
                candidate_cursor = checkpoint;
            }

            let ordered_replay_boundaries = [
                candidate_receipt.replay_round_checkpoints[0],
                candidate_receipt.replay_round_checkpoints[1],
                candidate_receipt.normalization_checkpoints[0],
                candidate_receipt.replay_round_checkpoints[2],
                candidate_receipt.normalization_checkpoints[1],
                candidate_receipt.replay_round_checkpoints[3],
            ];
            for checkpoint in ordered_replay_boundaries {
                candidate_sim.apply_iter(candidate_ops[candidate_cursor..checkpoint].iter());
                replay_sign_mismatch += candidate_sim
                    .qubit(candidate_receipt.replay_sign)
                    .count_ones() as u64;
                normalization_flag_mismatch += candidate_sim
                    .qubit(candidate_receipt.normalization_flag)
                    .count_ones() as u64;
            ...
            assert_eq!(candidate_cursor, candidate_receipt.replay_midpoint);
            let candidate_coefficient_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_coefficient_reg, shot))
                .collect();
            let candidate_numerator_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_numerator_reg, shot))
                .collect();

            candidate_sim.apply_iter(
                candidate_ops[candidate_cursor..candidate_receipt.replay_reverse_checkpoint]
                    .iter(),
            );
            for shot in 0..64 {
                if !candidate_sim
                    .get_register(&candidate_coefficient_reg, shot)
                    .is_zero()
                    || candidate_sim.get_register(&candidate_numerator_reg, shot)
                        != numerators[shot]
                {
                    replay_reverse_mismatch += 1;
                }
            }
            replay_sign_mismatch += candidate_sim
                .qubit(candidate_receipt.replay_sign)
            ...
            normalization_flag_mismatch += candidate_sim
                ...
                .count_ones() as u64;
            candidate_batch_phase_mask |= candidate_sim.phase;
            let candidate_reverse_coefficient_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_coefficient_reg, shot))
                .collect();
            let candidate_reverse_numerator_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_numerator_reg, shot))
                .collect();
            candidate_sim.apply_iter(
                candidate_ops[candidate_receipt.replay_reverse_checkpoint..].iter(),
            );
            let candidate_final_denominator_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_denominator_reg, shot))
                .collect();
            let candidate_final_numerator_values: Vec<U256> = (0..64)
                .map(|shot| candidate_sim.get_register(&candidate_numerator_reg, shot))
                .collect();
            for shot in 0..64 {
                if candidate_final_denominator_values[shot] != denominators[shot]
                    || candidate_final_numerator_values[shot] != numerators[shot]
                {
                    candidate_final_classical += 1;
                }
            }
            candidate_batch_phase_mask |= candidate_sim.phase;
            candidate_executed_toffoli += candidate_sim.stats.toffoli_gates;
            candidate_phase_lanes += candidate_batch_phase_mask.count_ones() as u64;

            let mut reference_shake = Shake256::default();
            reference_shake.update(b"Teddy Pender paired production-prefix replay");
            ...
            for shot in 0..64 {
                reference_sim.set_register(
                    &reference_denominator_reg,
                    denominators[shot],
                    shot,
                );
                reference_sim.set_register(&reference_numerator_reg, numerators[shot], shot);
            }
            let mut reference_batch_phase_mask = 0u64;
            reference_sim.apply_iter(
                reference_ops[..reference_receipt.replay_midpoint].iter(),
            );
            reference_batch_phase_mask |= reference_sim.phase;
            for shot in 0..64 {
                let reference_coefficient =
                    reference_sim.get_register(&reference_coefficient_reg, shot);
                let reference_numerator =
                    reference_sim.get_register(&reference_numerator_reg, shot);
                let coefficient_differs =
                    candidate_coefficient_values[shot] != reference_coefficient;
                let numerator_differs = candidate_numerator_values[shot] != reference_numerator;
                continuation_coefficient_mismatch += u64::from(coefficient_differs);
                continuation_numerator_mismatch += u64::from(numerator_differs);
                continuation_mismatch += u64::from(coefficient_differs || numerator_differs);
            }
            reference_sim.apply_iter(
                reference_ops[reference_receipt.replay_midpoint
                    ..reference_receipt.replay_reverse_checkpoint]
                    .iter(),
            );
            for shot in 0..64 {
                let coefficient_differs = candidate_reverse_coefficient_values[shot]
                    != reference_sim.get_register(&reference_coefficient_reg, shot);
                let numerator_differs = candidate_reverse_numerator_values[shot]
                    != reference_sim.get_register(&reference_numerator_reg, shot);
                paired_replay_reverse_mismatch +=
                    u64::from(coefficient_differs || numerator_differs);
            }
            reference_sim.apply_iter(
                reference_ops[reference_receipt.replay_reverse_checkpoint..].iter(),
            );
            for shot in 0..64 {
                let reference_final_denominator =
                    reference_sim.get_register(&reference_denominator_reg, shot);
                let reference_final_numerator =
                    reference_sim.get_register(&reference_numerator_reg, shot);
                if reference_final_denominator != denominators[shot]
                    || reference_final_numerator != numerators[shot]
                {
                    reference_final_classical += 1;
                }
                paired_final_classical_mismatch += u64::from(
                    reference_final_denominator != candidate_final_denominator_values[shot]
                        || reference_final_numerator != candidate_final_numerator_values[shot],
                );
            }
            reference_batch_phase_mask |= reference_sim.phase;
            reference_executed_toffoli += reference_sim.stats.toffoli_gates;
            reference_phase_lanes += reference_batch_phase_mask.count_ones() as u64;
            paired_final_phase_xor_lanes +=
                (candidate_sim.phase ^ reference_sim.phase).count_ones() as u64;

            for register in [&candidate_denominator_reg, &candidate_numerator_reg] {
            ...
            for q in 0..candidate_total_qubits as u64 {
                candidate_ancilla_wire_lanes +=
                    candidate_sim.qubit(QubitId(q)).count_ones() as u64;
            }
            for register in [&reference_denominator_reg, &reference_numerator_reg] {
                for wire in register {
                    if let QubitOrBit::Qubit(q) = *wire {
            ...
            for q in 0..reference_total_qubits as u64 {
                reference_ancilla_wire_lanes +=
                    reference_sim.qubit(QubitId(q)).count_ones() as u64;
            }
        }
    }

    let candidate_phase_count = candidate_phase_lanes;
    let reference_phase_count = reference_phase_lanes;
    let operation_delta = candidate_ops.len() as i64 - reference_ops.len() as i64;
    let emitted_toffoli_delta =
        candidate_emitted_toffoli as i64 - reference_emitted_toffoli as i64;
    let executed_toffoli_delta_total =
        candidate_executed_toffoli as i128 - reference_executed_toffoli as i128;
    let replay_schedule = if std::env::var_os(
        "SUB4_PP_RETAINED_PRODUCTION_PREFIX_PRODUCTION_FAITHFUL",
    )
    .is_some()
    {
    ...
    let round_growing_state = u64::from(
        candidate_receipt
            .forward_persistent_active
            .windows(2)
    ...
    const PRODUCTION_PREFIX_WIDTH_CEILING: u32 = 1182;
    let peak_width_growth = candidate_peak.saturating_sub(reference_peak);
    let live_numerator_cleanup_failure = live_numerator_stress
        && (replay_reverse_mismatch != 0
            || candidate_final_classical != 0
            || candidate_phase_count != 0
            || candidate_ancilla_wire_lanes != 0
            || reference_final_classical != 0
            || reference_phase_count != 0
            || reference_ancilla_wire_lanes != 0);
    let live_numerator_abi_clean =
        u64::from(live_numerator_stress && !live_numerator_cleanup_failure);
    let paired_verdict = if stored_round_zero_mismatch != 0
        || forward_sign_mismatch != 0
        || replay_sign_mismatch != 0
        || normalization_flag_mismatch != 0
        || retained_state_mismatch != 0
        || round_growing_state != 0
    ...
    let verdict = if stored_round_zero_mismatch != 0 {
        "KILL_STORED_ROUND_ZERO_MISMATCH"
    } else if forward_sign_mismatch != 0 {
        "KILL_FORWARD_SIGN_MISMATCH"
    } else if replay_sign_mismatch != 0 {
        "KILL_REPLAY_SIGN_MISMATCH"
    } else if normalization_flag_mismatch != 0 {
        "KILL_NORMALIZATION_FLAG"
    } else if retained_state_mismatch != 0 {
        "KILL_RETAINED_LOW_FIVE"
    } else if round_growing_state != 0 {
        "KILL_ROUND_GROWING_STATE"
    } else if continuation_mismatch != 0 {
        "KILL_CONTINUATION_MISMATCH"
    } else if live_numerator_cleanup_failure {
        "KILL_LIVE_NUMERATOR_ABI_CLOSURE"
    } else if candidate_phase_count != 0 {
        "KILL_PHASE_DEBT"
    } else if candidate_ancilla_wire_lanes != 0 {
        "KILL_ANCILLA_DEBT"
    } else if replay_reverse_mismatch != 0 || candidate_final_classical != 0 {
        "KILL_COMPONENT_ABI"
    } else if reference_phase_count != 0
        || reference_ancilla_wire_lanes != 0
        || reference_final_classical != 0
    {
        "KILL_UNTRUSTED_REFERENCE"
    } else if candidate_peak > PRODUCTION_PREFIX_WIDTH_CEILING {
        "KILL_Q1182_WIDTH_CEILING"
    } else {
        "HOLD_LOW5_PRODUCTION_PREFIX"
    };

    eprintln!(
        "TEDDY_LOW5_PRODUCTION_PREFIX verdict={verdict} paired_verdict={paired_verdict} schedule={replay_schedule} rounds=0..3 replaced_signs=1..3 retained_q=5 oracle_scratch_q=0 lanes={case_count} seed_mode={seed_mode} width_ceiling_q={PRODUCTION_PREFIX_WIDTH_CEILING} production_semantics_closed=0 live_numerator_abi_clean={live_numerator_abi_clean} phase_scope=component_diagnostic_not_promotion_authority candidate_peak_q={candidate_peak} candidate_peak_binder={candidate_peak_phase} candidate_abi_q={candidate_abi} candidate_extra_peak_q={} candidate_total_q={candidate_total_qubits} candidate_classical_bits={candidate_total_bits} candidate_ops={} candidate_emitted_t={candidate_emitted_toffoli} candidate_executed_t={:.6} candidate_executed_t_total={candidate_executed_toffoli} reference_peak_q={reference_peak} reference_peak_binder={reference_peak_phase} reference_abi_q={reference_abi} reference_total_q={reference_total_qubits} reference_classical_bits={reference_total_bits} reference_ops={} reference_emitted_t={reference_emitted_toffoli} reference_executed_t={:.6} reference_executed_t_total={reference_executed_toffoli} peak_width_growth={peak_width_growth} operation_delta={operation_delta} emitted_t_delta={emitted_toffoli_delta} executed_t_delta_total={executed_toffoli_delta_total} stored_round_zero_mismatch={stored_round_zero_mismatch} forward_sign_mismatch={forward_sign_mismatch} replay_sign_mismatch={replay_sign_mismatch} continuation_mismatch={continuation_mismatch} continuation_coefficient_mismatch={continuation_coefficient_mismatch} continuation_numerator_mismatch={continuation_numerator_mismatch} replay_reverse_mismatch={replay_reverse_mismatch} paired_replay_reverse_mismatch={paired_replay_reverse_mismatch} retained_state_mismatch={retained_state_mismatch} normalization_flag_mismatch={normalization_flag_mismatch} concurrent_normalization_flags=0 round_growing_state={round_growing_state} forward_persistent_active={:?} replay_persistent_active={:?} candidate_classical={candidate_final_classical} reference_classical={reference_final_classical} paired_final_classical_mismatch={paired_final_classical_mismatch} candidate_phase={candidate_phase_count} reference_phase={reference_phase_count} paired_final_phase_xor_lanes={paired_final_phase_xor_lanes} candidate_ancilla={candidate_ancilla_wire_lanes} reference_ancilla={reference_ancilla_wire_lanes} model='Kimi Code (k2/k3, high effort), multi-lane agent campaign' credit='Teddy Pender supplied the five-bit sufficient statistic, tape-removal architecture, and Burn-the-House-Down method'",
        candidate_peak - candidate_abi,
        candidate_ops.len(),
        candidate_executed_toffoli as f64 / case_count as f64,
        reference_ops.len(),
        reference_executed_toffoli as f64 / case_count as f64,
        candidate_receipt.forward_persistent_active,
        candidate_receipt.replay_persistent_active,
    );
    if !live_numerator_stress {
        assert_eq!(
            verdict, "HOLD_LOW5_PRODUCTION_PREFIX",
            "Teddy's bounded production integration gate changed verdict"
        );
    }
}

/// Exact retained-word signs for Teddy Pender's round-3 descent. Rounds one
/// through three are linear; the unrelated sign-4-through-7 ANF decoder stays
/// on the evidence branch. The scratch pair remains live so the new receipt is
/// width-comparable with evidence commit 40d0170.
fn teddy_retained_sign_1_to_3_oracle(
    b: &mut B,
    retained: &[QubitId],
    round: usize,
    scratch: &[QubitId; 2],
    ...
    debug_assert!(scratch.iter().all(|q| !retained.contains(q)));
    b.x(sign);
    match round {
        1 => b.cx(retained[2], sign),
        2 => b.cx(retained[3], sign),
        3 => {
            for &index in &[0usize, 2, 3, 4] {
                b.cx(retained[index], sign);
            }
        }
        _ => panic!("Teddy retained-word diagnostic covers signs 1 through 3"),
    }
}

fn teddy_retained_toggle_round1_sentinel(
    b: &mut B,
    retained: &[QubitId],
    ...
    teddy_retained_sign_1_to_3_oracle(
        b,
        retained,
        1,
        oracle_scratch,
...
fn teddy_production_toggle_round1_sentinel(
    b: &mut B,
    sign1: QubitId,
    normalization_flag: QubitId,
    x: &[QubitId],
...
/// Preserve the exact 4,096-denominator census from the evidence receipt.
fn teddy_retained_round3_high_prefixes() -> Vec<U256> {
...
/// Teddy Pender's smallest environment-gated round-3 retained-word probe on
/// the promoted dead-low source. It retains one denominator word, reconstructs
/// one sign at a time, and clears the sole normalization flag at every bounded
/// boundary. The accepted point-add path cannot call this function.
pub(crate) fn teddy_retained_round3_selfcheck() {
    use crate::circuit::QubitOrBit;
    use sha3::{
        digest::{ExtendableOutput, Update},
        Shake256,
    ...
    clear_chunks();
    let mut candidate = B::new();
    let denominator = candidate.alloc_qubits(N);
    let x = candidate.alloc_qubits(N);
    let y = candidate.alloc_qubits(N);
    let candidate_abi = candidate.active_qubits;
    let retained = candidate.alloc_qubits(N);
    for i in 0..N {
        candidate.cx(denominator[i], retained[i]);
    }
    ...
    candidate.set_phase("teddy_retained_round0_mod_halve_pm");
    replay_halving_round(&mut candidate, 0, sign, &x, &y);
    let round0_checkpoint = candidate.ops.len();
    let mut round_checkpoints = [0usize; 3];
    let mut normalization_checkpoints = [0usize; 2];

    candidate.set_phase("teddy_retained_round1_seed_and_mod_halve_pm");
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 1, &scratch, sign);
    replay_halving_round(&mut candidate, 1, sign, &x, &y);
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 1, &scratch, sign);
    round_checkpoints[0] = candidate.ops.len();

    candidate.set_phase("teddy_retained_normalize_before_round2");
    teddy_retained_toggle_round1_sentinel(
        &mut candidate,
        &retained,
        &scratch,
        normalization_flag,
    ...
    normalization_checkpoints[0] = candidate.ops.len();
    candidate.set_phase("teddy_retained_round2_signed_mod_add_pm_halve_fused");
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 2, &scratch, sign);
    replay_halving_round(&mut candidate, 2, sign, &x, &y);
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 2, &scratch, sign);
    candidate.set_phase("teddy_retained_normalize_before_round3");
    teddy_retained_toggle_round1_sentinel(
        &mut candidate,
        &retained,
        &scratch,
    ...
    round_checkpoints[1] = candidate.ops.len();
    ...
    normalization_checkpoints[1] = candidate.ops.len();
    candidate.set_phase("teddy_retained_round3_signed_mod_add_pm_halve_fused");
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 3, &scratch, sign);
    replay_halving_round(&mut candidate, 3, sign, &x, &y);
    teddy_retained_sign_1_to_3_oracle(&mut candidate, &retained, 3, &scratch, sign);
    teddy_retained_toggle_round1_sentinel(
        &mut candidate,
        &retained,
    ...
    round_checkpoints[2] = candidate.ops.len();

    candidate.set_phase("teddy_retained_round3_cleanup");
    ...
    let candidate_peak = candidate.peak_qubits;
    let candidate_peak_ops_idx = candidate.peak_ops_idx;
    let candidate_peak_phase = candidate.peak_phase;
    let candidate_total_qubits = candidate.next_qubit as usize;
    let candidate_total_bits = candidate.next_bit as usize;
    let candidate_ops = candidate.take_ops();
    let candidate_emitted_t = candidate_ops
        .iter()
        .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ))
        .count();
    let mut emitted_cursor = 0usize;
    let round_emitted_t = [round0_checkpoint, round_checkpoints[0], round_checkpoints[1], round_checkpoints[2]]
        .map(|checkpoint| {
            let count = candidate_ops[emitted_cursor..checkpoint]
                .iter()
                .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ))
                .count();
            emitted_cursor = checkpoint;
            count
        });

    // Independent exact reference: production walk signs, identical replay
    // cells, then exact walk reversal through the full round-zero endpoint.
    clear_walk_peak();
    clear_chunks();
    let mut reference = B::new();
    let reference_denominator = reference.alloc_qubits(N);
    ...
    let reference_flag = reference.alloc_qubit();
    teddy_production_toggle_round1_sentinel(&mut reference, sign1, reference_flag, &reference_x);
    replay_halving_round(&mut reference, 2, sign2, &reference_x, &reference_y);
    teddy_production_toggle_round1_sentinel(&mut reference, sign1, reference_flag, &reference_x);
    teddy_production_toggle_round1_sentinel(&mut reference, sign1, reference_flag, &reference_x);
    replay_halving_round(&mut reference, 3, sign3, &reference_x, &reference_y);
    teddy_production_toggle_round1_sentinel(&mut reference, sign1, reference_flag, &reference_x);
    reference.free(reference_flag);
    ...
    teddy_fused_lift_round0_reverse_full(&mut reference, &v, sign0);
    for i in 0..N {
        reference.cx(reference_denominator[i], v[i]);
        if SECP256K1_P.bit(i) {
            reference.x(u[i]);
    ...
    let reference_total_bits = reference.next_bit as usize;
    let reference_ops = reference.take_ops();
    let reference_emitted_t = reference_ops
        .iter()
        .filter(|op| matches!(op.kind, OperationType::CCX | OperationType::CCZ))
        .count();

    let reg = |qubits: &[QubitId]| {
        qubits.iter().copied().map(QubitOrBit::Qubit).collect::<Vec<_>>()
    };
    let denominator_reg = reg(&denominator);
    let x_reg = reg(&x);
    let y_reg = reg(&y);
    let retained_reg = reg(&retained);
    let reference_denominator_reg = reg(&reference_denominator);
    let reference_x_reg = reg(&reference_x);
    let reference_y_reg = reg(&reference_y);
    let mut candidate_executed_t = 0u64;
    let mut reference_executed_t = 0u64;
    let mut round_executed_t = [0u64; 4];

    for (prefix, base) in teddy_retained_round3_high_prefixes().into_iter().enumerate() {
        for residue_batch in 0..8 {
            let batch = prefix * 8 + residue_batch;
            let start = residue_batch * 64;
            let denominators: Vec<U256> = (0..64)
                .map(|residue| base.wrapping_add(U256::from(start + residue)))
                .collect();

            let mut shake = Shake256::default();
            // Keep the evidence branch's XOF label so executed-Toffoli deltas
            // isolate source changes instead of changing measurement draws.
            shake.update(b"Teddy Pender retained replay slice candidate");
            shake.update(&(batch as u64).to_le_bytes());
            let mut reader = shake.finalize_xof();
            let mut sim = Simulator::new(candidate_total_qubits, candidate_total_bits, &mut reader);
            for (shot, &value) in denominators.iter().enumerate() {
                sim.set_register(&denominator_reg, value, shot);
            }
            sim.apply_iter(candidate_ops[..round0_checkpoint].iter());
            assert_eq!(sim.qubit(sign), 0);
            assert_eq!(sim.qubit(normalization_flag), 0);
            assert_eq!(sim.phase, 0);
            round_executed_t[0] += sim.stats.toffoli_gates;
            let mut toffoli_cursor = sim.stats.toffoli_gates;
            let mut cursor = round0_checkpoint;
            for (round_index, &checkpoint) in round_checkpoints.iter().enumerate() {
                sim.apply_iter(candidate_ops[cursor..checkpoint].iter());
                round_executed_t[round_index + 1] += sim.stats.toffoli_gates - toffoli_cursor;
                toffoli_cursor = sim.stats.toffoli_gates;
                assert_eq!(sim.qubit(sign), 0, "sign dirty after round {} batch {batch}", round_index + 1);
                assert_eq!(sim.qubit(normalization_flag), 0, "flag dirty after round {} batch {batch}", round_index + 1);
                assert_eq!(sim.qubit(scratch[0]), 0);
                assert_eq!(sim.qubit(scratch[1]), 0);
                assert_eq!(sim.phase, 0, "phase dirty after round {} batch {batch}", round_index + 1);
                for (shot, &value) in denominators.iter().enumerate() {
                    assert_eq!(sim.get_register(&retained_reg, shot), value);
                }
                cursor = checkpoint;
                if round_index < normalization_checkpoints.len() {
                    let boundary = normalization_checkpoints[round_index];
                    sim.apply_iter(candidate_ops[cursor..boundary].iter());
                    assert_eq!(sim.qubit(sign), 0);
                    assert_eq!(sim.qubit(normalization_flag), 0);
                    assert_eq!(sim.qubit(scratch[0]), 0);
                    assert_eq!(sim.qubit(scratch[1]), 0);
                    assert_eq!(sim.phase, 0);
                    for shot in 0..64 {
                        assert_eq!(sim.get_register(&x_reg, shot), U256::ZERO);
                        assert_eq!(sim.get_register(&y_reg, shot), U256::ZERO);
                        assert_eq!(sim.get_register(&retained_reg, shot), denominators[shot]);
                    }
                    cursor = boundary;
                }
            }
            sim.apply_iter(candidate_ops[cursor..].iter());
            let x_values: Vec<U256> = (0..64).map(|shot| sim.get_register(&x_reg, shot)).collect();
            let y_values: Vec<U256> = (0..64).map(|shot| sim.get_register(&y_reg, shot)).collect();
            for (shot, &value) in denominators.iter().enumerate() {
                let expected_x = if value.bit(2) { U256::ZERO } else { SECP256K1_P };
                assert_eq!(x_values[shot], expected_x);
                assert_eq!(y_values[shot], U256::ZERO);
                assert_eq!(sim.get_register(&denominator_reg, shot), value);
            }
            assert_eq!(sim.phase, 0);
            candidate_executed_t += sim.stats.toffoli_gates;

            let mut reference_shake = Shake256::default();
            reference_shake.update(b"Teddy Pender retained replay slice reference");
            reference_shake.update(&(batch as u64).to_le_bytes());
            let mut reference_reader = reference_shake.finalize_xof();
            let mut reference_sim = Simulator::new(reference_total_qubits, reference_total_bits, &mut reference_reader);
            for (shot, &value) in denominators.iter().enumerate() {
                reference_sim.set_register(&reference_denominator_reg, value, shot);
            }
            reference_sim.apply_iter(reference_ops.iter());
            for (shot, &value) in denominators.iter().enumerate() {
                assert_eq!(reference_sim.get_register(&reference_denominator_reg, shot), value);
                assert_eq!(reference_sim.get_register(&reference_x_reg, shot), x_values[shot]);
                assert_eq!(reference_sim.get_register(&reference_y_reg, shot), y_values[shot]);
            }
            assert_eq!(reference_sim.phase, 0);
            reference_executed_t += reference_sim.stats.toffoli_gates;

            for wire in denominator_reg.iter().chain(&x_reg).chain(&y_reg) {
                if let QubitOrBit::Qubit(q) = *wire { *sim.qubit_mut(q) = 0; }
            }
            for q in 0..candidate_total_qubits as u64 {
                assert_eq!(sim.qubit(QubitId(q)), 0, "candidate dirty q{q} batch {batch}");
            }
            for wire in reference_denominator_reg.iter().chain(&reference_x_reg).chain(&reference_y_reg) {
                if let QubitOrBit::Qubit(q) = *wire { *reference_sim.qubit_mut(q) = 0; }
            }
            for q in 0..reference_total_qubits as u64 {
                assert_eq!(reference_sim.qubit(QubitId(q)), 0, "reference dirty q{q} batch {batch}");
            }
        }
    }

    eprintln!(
        "TEDDY_RETAINED_ROUND3_9805DEE PASS rounds=0..3 reconstructed_signs=1..3 lanes=4096 low_residues=512 high_prefixes=8 candidate_peak_q={candidate_peak} candidate_peak_ops_idx={candidate_peak_ops_idx} candidate_peak_phase={candidate_peak_phase} candidate_abi_q={candidate_abi} candidate_extra_peak_q={} candidate_total_q={candidate_total_qubits} candidate_classical_bits={candidate_total_bits} candidate_ops={} candidate_emitted_t={candidate_emitted_t} candidate_round_emitted_t=0:{},1:{},2:{},3:{} candidate_executed_t={:.3} candidate_round_executed_t=0:{:.3},1:{:.3},2:{:.3},3:{:.3} reference_peak_q={reference_peak} reference_abi_q={reference_abi} reference_total_q={reference_total_qubits} reference_classical_bits={reference_total_bits} reference_ops={} reference_emitted_t={reference_emitted_t} reference_executed_t={:.3} denominator_preserved=1 retained_word_preserved=1 replay_state_match=1 normalization_flag_peak=1 normalization_flag_final=0 normalization_flag_cleared_before_sign=1 concurrent_normalization_flags=0 phase=0 ancilla=0 persistent_carrier_bits=0 max_live_sign_bits=1 fixed_oracle_scratch_q=2 normal_path_unchanged=1",
        candidate_peak - candidate_abi,
        candidate_ops.len(),
        round_emitted_t[0],
        round_emitted_t[1],
        round_emitted_t[2],
        round_emitted_t[3],
        candidate_executed_t as f64 / 4096.0,
        round_executed_t[0] as f64 / 4096.0,
        round_executed_t[1] as f64 / 4096.0,
        round_executed_t[2] as f64 / 4096.0,
        round_executed_t[3] as f64 / 4096.0,
        reference_ops.len(),
        reference_executed_t as f64 / 4096.0,
    );
}
...
/// Exact inverse of `fused_lift_round0_forward` for Teddy Pender's bounded
/// Burn-the-House-Down diagnostics. Production deliberately keeps its sparse
/// endpoint in `fused_lift_round0_reverse`; this full endpoint is reachable
/// only from the reference-cleanup self-check.
fn burn_fused_lift_round0_reverse_full(b: &mut B, v: &[QubitId], a0: QubitId) {
    debug_assert_eq!(v.len(), VALUE_WIDTH);
    let not_a1 = b.alloc_qubit();
    b.cx(v[VALUE_WIDTH - 1], not_a1);
    let f = U256::MAX
...
const BURN_REFERENCE_PREFIX_ROUNDS: usize = 4;

/// Operation boundaries for the unchanged four-round replay reference.  The
/// matched forward and reverse snapshots make the first bad inverse stage an
/// executable receipt instead of inferring it from a dirty final register.
struct BurnReferenceCleanupReceipt {
    walk_entry: usize,
    walk_forward: [usize; BURN_REFERENCE_PREFIX_ROUNDS],
    replay_forward: [usize; 8],
    replay_reverse: [usize; 8],
    replay_cleanup: usize,
    walk_reverse: [usize; BURN_REFERENCE_PREFIX_ROUNDS],
    final_checkpoint: usize,
    coefficient: Vec<QubitId>,
}

#[derive(Clone)]
struct BurnReferenceSnapshot {
    phase: u64,
    qubits: Vec<u64>,
}

struct BurnReferenceRun {
    schedule: &'static str,
    first_bad_stage: &'static str,
    first_bad_lanes: u64,
    first_bad_wire_lanes: u64,
    first_bad_phase_lanes: u64,
    first_bad_replay_logical_stage: &'static str,
    first_bad_replay_logical_lanes: u64,
    midpoint_phase_lanes: u64,
    final_classical_lanes: u64,
    final_phase_lanes: u64,
    final_ancilla_wire_lanes: u64,
    peak_qubits: u32,
    operations: usize,
...
fn burn_reference_toggle_round1_sentinel(
    b: &mut B,
    sign1: QubitId,
    normalization_flag: QubitId,
    x: &[QubitId],
...
/// Build the short reference twice: once with the historical diagnostic's
/// four round-one sentinels and once with the production-faithful replay.  No
/// low-five decoder appears here, so a failure cannot be charged to Teddy's
/// representation.
fn burn_build_reference_cleanup_slice(
    b: &mut B,
    denominator: &[QubitId],
    numerator: &[QubitId],
    with_sentinels: bool,
) -> BurnReferenceCleanupReceipt {
    assert_eq!(denominator.len(), N);
    assert_eq!(numerator.len(), N);
    assert!(fused_lift_round0_enabled());
    let entry_active = b.active_qubits;
    ...
    v.extend(b.alloc_qubits(VALUE_WIDTH - N));
    let wanted_v = v.clone();
    let walk_entry = b.ops.len();

    b.set_phase("burn_reference_walk");
    let mut tape = Vec::with_capacity(BURN_REFERENCE_PREFIX_ROUNDS);
    let mut walk_forward = [0usize; BURN_REFERENCE_PREFIX_ROUNDS];
    for (round, checkpoint) in walk_forward.iter_mut().enumerate() {
        tape.push(walk_round(b, &mut u, &mut v, round));
        *checkpoint = b.ops.len();
    }

    let coefficient = b.alloc_qubits(N);
    let coefficient_receipt = coefficient.clone();
    let normalization_flag = b.alloc_qubit();
    b.set_phase("burn_reference_replay");
    let mut replay_forward = [0usize; 8];
    replay_halving_round(b, 0, tape[0], &coefficient, numerator);
    replay_forward[0] = b.ops.len();
    replay_halving_round(b, 1, tape[1], &coefficient, numerator);
    replay_forward[1] = b.ops.len();
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
            normalization_flag,
            &coefficient,
    ...
    replay_forward[2] = b.ops.len();
    replay_halving_round(b, 2, tape[2], &coefficient, numerator);
    replay_forward[3] = b.ops.len();
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
    ...
    replay_forward[5] = b.ops.len();
    replay_halving_round(b, 3, tape[3], &coefficient, numerator);
    replay_forward[6] = b.ops.len();
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
    ...
    replay_forward[7] = b.ops.len();

    b.set_phase("burn_reference_replay_reverse");
    let mut replay_reverse = [0usize; 8];
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
    ...
    replay_reverse[0] = b.ops.len();
    replay_doubling_round(b, 3, tape[3], &coefficient, numerator);
    replay_reverse[1] = b.ops.len();
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
    ...
    replay_reverse[3] = b.ops.len();
    replay_doubling_round(b, 2, tape[2], &coefficient, numerator);
    replay_reverse[4] = b.ops.len();
    if with_sentinels {
        burn_reference_toggle_round1_sentinel(
            b,
            tape[1],
    ...
    replay_reverse[5] = b.ops.len();
    replay_doubling_round(b, 1, tape[1], &coefficient, numerator);
    replay_reverse[6] = b.ops.len();
    replay_doubling_round(b, 0, tape[0], &coefficient, numerator);
    replay_reverse[7] = b.ops.len();
    b.free(normalization_flag);
    b.free_vec(&coefficient);
    let replay_cleanup = b.ops.len();

    b.set_phase("burn_reference_walk_reverse");
    let mut walk_reverse = [0usize; BURN_REFERENCE_PREFIX_ROUNDS];
    for (index, round) in (1..BURN_REFERENCE_PREFIX_ROUNDS).rev().enumerate() {
        let sign = tape.pop().expect("reference tape holds bounded prefix sign");
        walk_back_round(b, &mut u, &mut v, round, sign);
        walk_reverse[index] = b.ops.len();
    }
    let round_zero = tape.pop().expect("reference tape holds round-zero sign");
    grow_to(b, &mut u, &mut v, VALUE_WIDTH);
    burn_fused_lift_round0_reverse_full(b, &v, round_zero);
    walk_reverse[3] = b.ops.len();
    burn_finish_bounded_walk(b, &mut u, &mut v, &wanted_u, &wanted_v);
    let final_checkpoint = b.ops.len();
    assert!(tape.is_empty());
    assert_eq!(b.active_qubits, entry_active);
    ...
    BurnReferenceCleanupReceipt {
        ...
        walk_reverse,
        final_checkpoint,
        coefficient: coefficient_receipt,
    }
}

fn burn_snapshot<R: sha3::digest::XofReader>(
    sim: &Simulator<'_, R>,
) -> BurnReferenceSnapshot {
    BurnReferenceSnapshot {
        phase: sim.phase,
        qubits: sim.qubits.clone(),
    }
}

fn burn_snapshot_diff<R: sha3::digest::XofReader>(
    sim: &Simulator<'_, R>,
    expected: &BurnReferenceSnapshot,
    qubits: &[QubitId],
) -> (u64, u64, u64, u64) {
    let phase_mask = sim.phase ^ expected.phase;
    let mut lane_mask = phase_mask;
    let mut logical_lane_mask = 0u64;
    let mut wire_lane_mismatches = 0u64;
    for &qubit in qubits {
        let actual = sim.qubit(qubit);
        let wanted = expected.qubits[qubit.0 as usize];
        let diff = actual ^ wanted;
        lane_mask |= diff;
        logical_lane_mask |= diff;
        wire_lane_mismatches += diff.count_ones() as u64;
    }
    (
        lane_mask.count_ones() as u64,
        logical_lane_mask.count_ones() as u64,
        wire_lane_mismatches,
        phase_mask.count_ones() as u64,
    )
}

fn burn_run_reference_cleanup_batch(
    denominators: &[U256],
    numerators: &[U256],
    with_sentinels: bool,
) -> BurnReferenceRun {
    ...
    assert_eq!(denominators.len(), 64);
    assert_eq!(numerators.len(), 64);
    clear_walk_peak();
    clear_chunks();
    let mut circuit = B::new();
    let denominator = circuit.alloc_qubits(N);
    let numerator = circuit.alloc_qubits(N);
    let receipt = burn_build_reference_cleanup_slice(
        &mut circuit,
        &denominator,
        &numerator,
        with_sentinels,
    );
    let peak_qubits = circuit.peak_qubits;
    let num_qubits = circuit.next_qubit as usize;
    let num_bits = circuit.next_bit as usize;
    ...
    let denominator_reg = to_register(&denominator);
    let numerator_reg = to_register(&numerator);
    let replay_logical_qubits = receipt
        .coefficient
        .iter()
        .chain(&numerator)
        .copied()
        .collect::<Vec<_>>();
    let all_qubits = (0..num_qubits as u64).map(QubitId).collect::<Vec<_>>();
    let mut shake = Shake256::default();
    // Keep the measurement draw paired across schedules so the comparison
    // changes only the historical sentinels, not the simulator randomness.
    shake.update(b"Teddy Pender burn reference cleanup paired schedule");
    let mut reader = shake.finalize_xof();
    let mut sim = Simulator::new(num_qubits, num_bits, &mut reader);
    for shot in 0..64 {
        sim.set_register(&denominator_reg, denominators[shot], shot);
        sim.set_register(&numerator_reg, numerators[shot], shot);
    }

    let mut cursor = 0usize;
    sim.apply_iter(ops[cursor..receipt.walk_entry].iter());
    cursor = receipt.walk_entry;
    let walk_entry = burn_snapshot(&sim);
    let mut walk_forward = Vec::with_capacity(BURN_REFERENCE_PREFIX_ROUNDS);
    for checkpoint in receipt.walk_forward {
        sim.apply_iter(ops[cursor..checkpoint].iter());
        cursor = checkpoint;
        walk_forward.push(burn_snapshot(&sim));
    }
    let mut replay_forward = Vec::with_capacity(8);
    for checkpoint in receipt.replay_forward {
        sim.apply_iter(ops[cursor..checkpoint].iter());
        cursor = checkpoint;
        replay_forward.push(burn_snapshot(&sim));
    }
    let midpoint_phase_lanes = sim.phase.count_ones() as u64;

    let reverse_stage_names = [
        "undo_sentinel_d",
        "undo_replay_round_3",
        "undo_sentinel_c",
        "undo_sentinel_b",
    ...
    let mut first_bad_stage = "none";
    let mut first_bad_lanes = 0u64;
    let mut first_bad_wire_lanes = 0u64;
    let mut first_bad_phase_lanes = 0u64;
    let mut first_bad_replay_logical_stage = "none";
    let mut first_bad_replay_logical_lanes = 0u64;
    for (index, checkpoint) in receipt.replay_reverse.into_iter().enumerate() {
        sim.apply_iter(ops[cursor..checkpoint].iter());
        cursor = checkpoint;
        let expected = if index < 7 {
            &replay_forward[6 - index]
        } else {
            &walk_forward[3]
        };
        let (lanes, logical_lanes, wire_lanes, phase_lanes) =
            burn_snapshot_diff(&sim, expected, &replay_logical_qubits);
        if first_bad_stage == "none" && lanes != 0 {
            first_bad_stage = reverse_stage_names[index];
            first_bad_lanes = lanes;
            first_bad_wire_lanes = wire_lanes;
            first_bad_phase_lanes = phase_lanes;
        }
        if first_bad_replay_logical_stage == "none" && logical_lanes != 0 {
            first_bad_replay_logical_stage = reverse_stage_names[index];
            first_bad_replay_logical_lanes = logical_lanes;
        }
    }
    sim.apply_iter(ops[cursor..receipt.replay_cleanup].iter());
    cursor = receipt.replay_cleanup;
    let (lanes, _logical_lanes, wire_lanes, phase_lanes) =
        burn_snapshot_diff(&sim, &walk_forward[3], &all_qubits);
    if first_bad_stage == "none" && lanes != 0 {
        first_bad_stage = "free_replay_workspace";
        first_bad_lanes = lanes;
        first_bad_wire_lanes = wire_lanes;
        first_bad_phase_lanes = phase_lanes;
    }

    let walk_reverse_names = [
        "undo_walk_round_3",
        "undo_walk_round_2",
        "undo_walk_round_1",
        "undo_walk_round_0",
    ];
    for (index, checkpoint) in receipt.walk_reverse.into_iter().enumerate() {
        sim.apply_iter(ops[cursor..checkpoint].iter());
        cursor = checkpoint;
        let expected = if index < 3 {
            &walk_forward[2 - index]
        } else {
            &walk_entry
        };
        let (lanes, _logical_lanes, wire_lanes, phase_lanes) =
            burn_snapshot_diff(&sim, expected, &all_qubits);
        if first_bad_stage == "none" && lanes != 0 {
            first_bad_stage = walk_reverse_names[index];
            first_bad_lanes = lanes;
            first_bad_wire_lanes = wire_lanes;
            first_bad_phase_lanes = phase_lanes;
        }
    }
    sim.apply_iter(ops[cursor..receipt.final_checkpoint].iter());

    let mut final_classical_mask = 0u64;
    for shot in 0..64 {
        if sim.get_register(&denominator_reg, shot) != denominators[shot]
            || sim.get_register(&numerator_reg, shot) != numerators[shot]
        {
            final_classical_mask |= 1u64 << shot;
        }
    }
    let final_phase_lanes = sim.phase.count_ones() as u64;
    for register in [&denominator_reg, &numerator_reg] {
        for wire in register {
            if let QubitOrBit::Qubit(q) = *wire {
                *sim.qubit_mut(q) = 0;
            }
    ...
    let final_ancilla_wire_lanes = sim
        .qubits
        .iter()
    ...
    BurnReferenceRun {
        ...
        first_bad_stage,
        first_bad_lanes,
        first_bad_wire_lanes,
        first_bad_phase_lanes,
        first_bad_replay_logical_stage,
        first_bad_replay_logical_lanes,
        midpoint_phase_lanes,
        final_classical_lanes: final_classical_mask.count_ones() as u64,
        final_phase_lanes,
        final_ancilla_wire_lanes,
        peak_qubits,
        operations: ops.len(),
        emitted_toffoli,
        executed_toffoli: sim.stats.toffoli_gates,
    }
}

/// Teddy Pender's cheapest exact falsifier for the dirty unchanged reference
/// at `34c1b50`.  It uses the first deterministic 64-lane nonzero batch from
/// that receipt, localizes inverse failure stage-by-stage, and compares the
/// historical sentinel schedule with the production-faithful replay.  This is
/// a component diagnostic only: it cannot authorize grinding or submission.
pub(crate) fn burn_reference_cleanup_selfcheck() {
    let stride = SECP256K1_P >> 4usize;
    let base = (stride >> 12usize) << 12usize;
    let denominators: Vec<U256> = (0..64)
        .map(|residue| base.wrapping_add(U256::from(residue)))
        .collect();
    let numerators: Vec<U256> = denominators
        .iter()
        .enumerate()
        .map(|(shot, &denominator)| {
            let candidate = match shot {
                0 => U256::from(1),
                1 => SECP256K1_P.wrapping_sub(U256::from(1)),
                _ => denominator
                    .mul_mod(U256::from((shot % 251 + 2) as u64), SECP256K1_P)
                    .add_mod(U256::from(5), SECP256K1_P),
            };
            if candidate.is_zero() {
                U256::from(7)
    ...
    for run in [
        burn_run_reference_cleanup_batch(&denominators, &numerators, true),
        burn_run_reference_cleanup_batch(&denominators, &numerators, false),
    ] {
        eprintln!(
            "BURN_REFERENCE_CLEANUP source=6b5c82c rounds=4 lanes=64 seed=nonzero schedule={} replay_fold_w={} replay_flag_compare={} endpoint_fold_w={} inverse={} chunk_order={} first_bad_stage={} first_bad_lanes={} first_bad_wire_lanes={} first_bad_phase_lanes={} first_bad_replay_logical_stage={} first_bad_replay_logical_lanes={} midpoint_phase_lanes={} final_classical={} final_phase={} final_ancilla={} peak_q={} operations={} emitted_t={} executed_t={} model='Kimi Code (k2/k3, high effort), multi-lane agent campaign' credit='Teddy Pender supplied the tape-removal architecture, five-bit sufficient statistic, and Burn-the-House-Down discipline'",
            run.schedule,
            replay_fold_window(),
            replay_flag_compare(),
            endpoint_fold_window(),
            if std::env::var_os("SUB4_PINGPONG_UNFUSED_INVERSE").is_some() {
                "unfused"
            } else {
                "fused"
            },
            if std::env::var_os("SUB4_PP_LEGACY_CHUNK_ORDER").is_some() {
                "legacy"
            } else {
                "late_carry"
            },
            run.first_bad_stage,
            run.first_bad_lanes,
            run.first_bad_wire_lanes,
            run.first_bad_phase_lanes,
            run.first_bad_replay_logical_stage,
            run.first_bad_replay_logical_lanes,
            run.midpoint_phase_lanes,
            run.final_classical_lanes,
            run.final_phase_lanes,
            run.final_ancilla_wire_lanes,
            run.peak_qubits,
            run.operations,
            run.emitted_toffoli,
            run.executed_toffoli,
        );
    }
}
