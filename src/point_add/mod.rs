...
mod compact_check;
mod q794_compact_check;
mod q793_compact_check_r01;
mod sprint_stream_check;
pub mod venting;
...
const STREAM_OPS_MAGIC: &[u8; 8] = b"QECCOPSZ";
const STREAM_OP_BYTES: usize = 56;

struct OpStreamWriter {
...
impl OpStreamWriter {
    fn from_env() -> Option<Self> {
        let path = std::env::var_os("POINT_ADD_STREAM_OPS_PATH")?;
        Some(
            Self::new(Path::new(&path))
                .unwrap_or_else(|error| panic!("create streaming ops writer: {error}")),
        )
    }
    ...
    fn new(path: &Path) -> std::io::Result<Self> {
        ...
        writer.write_all(STREAM_OPS_MAGIC)?;
        writer.write_all(&0_u64.to_le_bytes())?;
        let level = std::env::var("ZSTD_LEVEL")
            .ok()
            .and_then(|value| value.trim().parse().ok())
            .unwrap_or(3);
        let encoder = zstd::stream::write::Encoder::new(writer, level)?;
        Ok(Self {
    ...
    fn write_op(&mut self, op: &Op) -> std::io::Result<()> {
        let mut record = [0_u8; STREAM_OP_BYTES];
        record[0..4].copy_from_slice(&(op.kind as u32).to_le_bytes());
        record[8..16].copy_from_slice(&op.q_control2.0.to_le_bytes());
        record[16..24].copy_from_slice(&op.q_control1.0.to_le_bytes());
        record[24..32].copy_from_slice(&op.q_target.0.to_le_bytes());
        record[32..40].copy_from_slice(&op.c_target.0.to_le_bytes());
        record[40..48].copy_from_slice(&op.c_condition.0.to_le_bytes());
        record[48..56].copy_from_slice(&op.r_target.0.to_le_bytes());
        self.encoder
            .as_mut()
            .expect("streaming ops encoder")
            .write_all(&record)?;
        self.count += 1;
        Ok(())
    }
    ...
    fn finish(mut self) -> std::io::Result<u64> {
        let encoder = self.encoder.take().expect("streaming ops encoder");
        let mut writer = encoder.finish()?;
        writer.flush()?;
        drop(writer);

        let mut header = OpenOptions::new().write(true).open(&self.temporary_path)?;
        header.seek(SeekFrom::Start(STREAM_OPS_MAGIC.len() as u64))?;
        header.write_all(&self.count.to_le_bytes())?;
        header.flush()?;
        drop(header);
...
mod q793_runtime_r01;

pub struct B {
    pub(crate) q793_templates: q793_runtime_r01::Cache,
    pub ops: Vec<Op>,
    pub(crate) sprint_sim:Option<sprint_stream_check::Check>,
    // Development-only exact sequence storage; repeated primitive blocks share RAM.
    pub(crate) compact_blocks: Option<Vec<std::sync::Arc<Vec<Op>>>>,
    pub count_only: bool,
    stream_writer: Option<OpStreamWriter>,
...
impl B {
    fn new() -> Self {
        Self {
            ops: Vec::new(),
            q793_templates: Default::default(),
            sprint_sim:None,
            compact_blocks: None,
            count_only: false,
            stream_writer: None,
    ...
    pub(crate) fn new_with_ops_capacity(ops_capacity: usize) -> Self {
        let mut b = Self::new();
        b.stream_writer = OpStreamWriter::from_env();
        if std::env::var("LOWQ_WHOLE_COMPACT_SELFTEST").ok().as_deref()==Some("1") {
            assert!(b.stream_writer.is_none(), "compact diagnostic cannot also stream");
            b.compact_blocks=Some(Vec::new());
        } else if b.stream_writer.is_none() {
    ...
    fn push_op(&mut self, op: Op) {
        ...
        let capturing = !self.count_only_capture_stack.is_empty();
        if !capturing {if let Some(check)=&mut self.sprint_sim{check.apply(std::slice::from_ref(&op));}}
        ...
        if !self.count_only {
            if let Some(writer) = &mut self.stream_writer {
                if !capturing {
                    writer
                        .write_op(&op)
                        .unwrap_or_else(|error| panic!("stream operation: {error}"));
                }
            } else {
                self.ops.push(op);
            }
    ...
    pub(crate) fn flush_compact_block(&mut self) {
        if self.compact_blocks.is_some() {
            assert!(self.count_only_capture_stack.is_empty());
            assert!(!self.count_only && !self.is_streaming() && self.fiat_hash.is_none());
            if !self.ops.is_empty() {
                self.compact_blocks.as_mut().unwrap().push(std::sync::Arc::new(std::mem::take(&mut self.ops)));
            }
        }
    }
    ...
    pub(crate) fn finish_stream_writer(&mut self) {
        let Some(writer) = self.stream_writer.take() else {
            return;
        };
        let expected = self.counted_ops as u64;
        let actual = writer
            .finish()
            .unwrap_or_else(|error| panic!("finish streaming ops writer: {error}"));
        assert_eq!(actual, expected, "streamed operation count");
        assert!(self.ops.is_empty(), "streaming builder retained operations");
    }

    /// Two CCX ops are the same gate under control exchange: CCX(a,b,t) == CCX(b,a,t).
    /// CCZ is symmetric in ALL THREE qubits. A field-by-field struct comparison misses
    /// every such pair, so normalise before comparing.
    fn same_gate(x: &Op, y: &Op) -> bool {
        if x.kind != y.kind || x.c_condition != y.c_condition || x.c_target != y.c_target {
            return false;
        }
        match x.kind {
            OperationType::CCX => {
                if x.q_target != y.q_target {
                    return false;
                }
                let (mut a, mut b) = (x.q_control1, x.q_control2);
                if a.0 > b.0 { std::mem::swap(&mut a, &mut b); }
                let (mut c, mut d) = (y.q_control1, y.q_control2);
                if c.0 > d.0 { std::mem::swap(&mut c, &mut d); }
                a == c && b == d
            }
            OperationType::CCZ => {
                let mut p = [x.q_control1.0, x.q_control2.0, x.q_target.0];
                let mut q = [y.q_control1.0, y.q_control2.0, y.q_target.0];
                p.sort_unstable();
                q.sort_unstable();
                p == q
            }
    ...
    /// Adjacent identical CCX pairs cancel (CCX.CCX = I). Ported verbatim from
    /// junpenglao's q=824 artifact (72813aa), which is 6.5x cheaper than this route at
    /// +11 qubits and applies this post-pass; it is absent here entirely. Exact and
    /// route-independent -- it rewrites the emitted stream, so it re-rolls the
    /// Fiat-Shamir draw and needs a fresh tail nonce, but it cannot change semantics.
    fn cancel_adjacent_ccx_in_memory(ops: &mut Vec<Op>) -> usize {
        let mut write = 0usize;
        let mut removed = 0usize;
        for read in 0..ops.len() {
            let op = ops[read];
            if op.kind == OperationType::CCX && write != 0 && Self::same_gate(&ops[write - 1], &op) {
                write -= 1;
                removed += 2;
            } else {
                ops[write] = op;
    ...
    /// Does `o` commute with a CCX(a, b, t)?  Conservative and exact:
    ///   * o must not WRITE either control (that would change what the CCX computes),
    ///   * o must not READ the target (it would see a different value),
    ///   * if o writes the target it must be XOR-type (X/CX/CCX), since XORs on a
    ///     shared target commute with each other.
    /// Condition-stack ops are barriers: they change which shots execute.
    /// Measurement/reset (R, Hmr) and Swap are never treated as commuting writers.
    fn ccx_commutes_with(a: QubitId, b: QubitId, t: QubitId, o: &Op) -> bool {
        use OperationType::*;
        match o.kind {
            PushCondition | PopCondition => false,
            Register | AppendToRegister | DebugPrint | Neg
            | BitInvert | BitStore0 | BitStore1 => true,
            X => o.q_target != a && o.q_target != b,
            CX => {
                o.q_target != a && o.q_target != b && o.q_control1 != t
            }
            CCX => {
                o.q_target != a && o.q_target != b
                    && o.q_control1 != t && o.q_control2 != t
            }
            Z => o.q_target != t,
            CZ => o.q_control1 != t && o.q_target != t,
            CCZ => o.q_control1 != t && o.q_control2 != t && o.q_target != t,
            // Swap writes both operands; R/Hmr are non-unitary. Never commute past them.
            Swap | R | Hmr => {
                o.q_target != a && o.q_target != b && o.q_target != t
                    && o.q_control1 != a && o.q_control1 != b && o.q_control1 != t
            }
        }
    }

    /// Cancel identical CCX pairs separated by ops that provably commute with them.
    /// The adjacent-only pass below is the `window == 0` case of this; widening the
    /// window is the "exact rewrites across N intervening gates" lever. Exact: it only
    /// removes pairs whose intervening ops commute, so the unitary is unchanged.
    fn cancel_commuting_ccx_in_memory(ops: &mut Vec<Op>, window: usize) -> usize {
        let n = ops.len();
        let mut dead = vec![false; n];
        let mut removed = 0usize;
        for i in 0..n {
            if dead[i] || ops[i].kind != OperationType::CCX {
                continue;
            }
            let (a, b, t) = (ops[i].q_control1, ops[i].q_control2, ops[i].q_target);
            let hi = (i + 1 + window).min(n);
            for j in (i + 1)..hi {
                if dead[j] {
                    continue;
                }
                if Self::same_gate(&ops[j], &ops[i]) {
                    dead[i] = true;
                    dead[j] = true;
                    removed += 2;
                    break;
                }
                if !Self::ccx_commutes_with(a, b, t, &ops[j]) {
                    break;
                }
            }
        }
    ...
    /// Exact bulk append when no per-operation observer is installed.
    pub(crate) fn append_nct_template(&mut self, ops:&[Op], kinds:&[usize;18]) {
        assert_eq!(kinds.iter().sum::<usize>(),ops.len());
        assert_eq!(kinds[OperationType::X as usize]+kinds[OperationType::CX as usize]+kinds[OperationType::CCX as usize],ops.len());
        if !self.count_only && self.stream_writer.is_none() && self.fiat_hash.is_none()
            && self.sprint_sim.is_none() && self.count_only_capture_stack.is_empty() && self.compact_blocks.is_none() {
            self.ops.extend_from_slice(ops);
            for kind in [OperationType::X,OperationType::CX,OperationType::CCX] {self.add_counted_kind(kind,kinds[kind as usize]);}
        } else {for &op in ops {self.push_op(op);}}
    }
    ...
    fn current_ops_len(&self) -> usize {
        if self.count_only || self.is_streaming() || self.compact_blocks.is_some() {
            self.counted_ops
        } else {
            self.ops.len()
        }
...
fn configure_ecdsafail_submission_route() {
    ...
    set_default_env("DIALOG_GCD_SELECTED_BODY_STREAM_SUFFIX_MAP", "3:2,4:3,5:5,6:6,7:7,8:5,9:7,10:5,11:7,12:6,13:7,14:5,15:6,16:3,17:5,18:1,19:3,21:1");  // BAKED: gpt-5 1285q peak-drop (stream selected high bits through low-qubit suffix)
...
pub fn build() -> Vec<Op> {
    if std::env::var("LOWQ_CODEC_AUPDATE5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_Aupdate5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_GUARD_PAIR_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::conditional_pair::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R00_PHASED5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder5_phased::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_T11_PHASED5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_phase115_phased::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ROTATION5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_rotation5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_METADATA_PIPELINE5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_pipeline5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_PHASE_COUNTER5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_phase_counter5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_FULL_BOUNDARY5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_full_boundary5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_EXIT_BOUNDARY5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_exit_boundary5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ENTRY_BOUNDARY5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_entry_boundary5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ENTRY_HEAD5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_entry_head5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_TRANSFER5_COMPACT_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_transfer5_compact::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_TRANSFER5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_transfer5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_PHASE115_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_phase115::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R015_FUNDED_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder015_funded::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R015_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder015::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R005_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_TERMINAL5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_terminal5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_T10_ENCODED5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_arithmetic5_encoded::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_RANK5_LIFECYCLE_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::rank5_lifecycle::check::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_CORE5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_core5_check::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_FULL_STEP5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_full_step5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ARITH_PHASED5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_arithmetic5_phased::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ARITH5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_arithmetic5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_COUNTER5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_counter5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_RANK5_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_rank5::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R01_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder01::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_R00_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_remainder::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_PHASE11_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_phase11::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_PHASE10_T_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_coefficient_block::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_FUSED_LOAN_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_address_swap::run_fused_loan();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ZERO_LOAN_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_address_swap::run_zero_loan();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_ADDRESS_SWAP_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_address_swap::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_SUM_QUERY_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_sum_query::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_COUNTER_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_step_counter::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_COMPRESSION_PROBE").ok().as_deref()==Some("1") {
        return trailmix_port::inversion::paper2607_eea::shared_lifecycle_check::compression_probe_ops();
    }
    if std::env::var("LOWQ_CODEC_QUERY_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_rank::run_queries();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CODEC_RANK_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::metadata_rank::run();
        std::process::exit(0);
    }
    if std::env::var("LOWQ_CONDITIONAL_MCX_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::conditional_mcx::verification::run();
        return Vec::new();
    }
    if std::env::var("LOWQ_SHARED_LIFECYCLE_SELFTEST").ok().as_deref()==Some("1") {
        trailmix_port::inversion::paper2607_eea::shared_lifecycle_check::run();
        return Vec::new();
    }
    if std::env::var("LOWQ_FULL_INVERSION_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_inversion_check::run();
        return Vec::new();
    }
    if std::env::var("LOWQ_SCHEDULED_STEP_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_scheduled_steps();
        return Vec::new();
    }
    if std::env::var("LOWQ_ACTIVE_STEP_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_active_steps();
        return Vec::new();
    }
    if std::env::var("LOWQ_INTERVAL_ARITHMETIC_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_remainder::verification::run();
        return Vec::new();
    }
    if std::env::var("LOWQ_REMAINDER_BLOCK_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_remainders();
        return Vec::new();
    }
    if std::env::var("LOWQ_COEFFICIENT_BLOCK_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_coefficients();
        return Vec::new();
    }
    if std::env::var("LOWQ_PREFIX_ARITHMETIC_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_arithmetic::verification::run();
        return Vec::new();
    }
    if std::env::var("LOWQ_METADATA_BOUNDARY_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_boundaries();
        return Vec::new();
    }
    if std::env::var("LOWQ_METADATA_EXIT_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::shared_metadata::verification::run_exits();
        return Vec::new();
    }
    if std::env::var("LOWQ_METADATA_TRANSFER_SELFTEST").ok().as_deref() == Some("1") {
    ...
    if std::env::var("LOWQ_WINDOW_LENGTH_SELFTEST").ok().as_deref() == Some("1") {
        trailmix_port::inversion::length_recompute::verification::run_windows();
        return Vec::new();
    }
    // Rank5 candidate: full lifecycle verified separately; whole native and
    // official verification are required before publication.
    set_default_env("LOWQ_RANK5_EEA", "1");
    // Own Pareto candidate: exact modulo4 chart, two physically omitted
    // residual bits. C08 native lifecycle+independent whole64 PASS Q794.
    set_default_env("LOWQ_Q794_EEA", "1");
    // Own three-hole modulo8 implementation; exact source-default count.
    set_default_env("LOWQ_Q793_EEA", "1");
    // Q799 budget-repaired candidate: exact reversible routing and prefix
    // controls; no measurement, truncation, or nonce term added.
    for name in ["Q799_MUX_LEASE","Q799_MUX_QUOTIENT","Q799_XOR_LO","Q799_PREFIX_TREE","Q799_T11_FIRST","Q799_HEAD_TREE"]{set_default_env(name,"1");}
    set_default_env("Q799_T11_COMPARE","0"); // Valid comparator, but not cheaper here.
    set_default_env("Q799_CANCEL_WINDOW","2048");
    // Retain four invariant parity bits instead of five; physically omit r0.
    // Outer canonical arithmetic independently borrows clean top-bit flags.
    set_default_env("Q796_PARITY","1");
    // Q795: borrow both phase rails, with exact target-top T11 cargo handling.
    set_default_env("Q795_PHASE_LOAN","1");
    set_default_env("Q795_COMPARE_PEBBLE","1");
    // Q793 down-move: wider exact cancel window and reversed dirty-lender order (both exact, width-neutral; measured by t-census + 64-shot whole-stream).
    set_default_env("Q795_CORE_CANCEL","16384");
    set_default_env("LOWQ_MCX_REVERSE_ORDER","1");
    // Q793 down-move v2: affine (CNOT-conjugated) single-toggle transpositions replacing the remaining
    // Gray-walk-and-back copies on the Q793 step path (entry head permutation, R01 timefix short
    // permutation, post-entry Sign code). Same map, 1 toggle instead of 2d-1; t-census + 64-shot whole-stream.
    for name in ["Q793_AFFINE_ENTRY_HEAD","Q793_AFFINE_R01_TRANSPOSE","Q793_AFFINE_SIGN_CODE","Q793_R01_NUMERIC_SEED"]{set_default_env(name,"1");}
    // Q793 down-move v3: grouped exact dirty-echo factoring of the R01 timefix rank guards
    // (q793_r01_dynamic_timefix_r01::grouped; cost-model guarded, cannot regress).
    set_default_env("Q793_RANK_ECHO_R01","1");
    // Q793 lean R01: the Q794 lineage's closed numeric A/C/S carry scan (partial86
    // converter on rank5+HS, eight-bit M, A7 prefix cache) ported onto the three-hole
    // timefix core; selected per template by exact raw operation count.
    for name in ["Q793_R01_NUMERIC_SCAN","Q793_R01_A7_PREFIX","Q793_R01_NUMERIC_SEED1"]{set_default_env(name,"1");}
    // Exact rank-function dirty echo for repeated C1 and boundary routing
    // predicate banks. Full 32-code truth tables; arbitrary scratch restored;
    // direct emission wins ties.
    set_default_env("Q793_RANK_ECHO_STEP","1");
    // Keep the exact C1 rank chart in one arbitrary lender across its cargo
    // consumer, then restore it.  This removes one chart compute/uncompute pair.
    set_default_env("Q793_C1_HOLD","1");
    // Hold the exact j=2 C1 transition rank set across its cargo consumer.
    set_default_env("Q793_C1_J2_HOLD","1");
    // Hold one exact C-high-zero chart across the paired C1/C2 Sign mask loan.
    set_default_env("Q793_SIGN_C12_HOLD","1");
    set_default_env("Q793_RANK_ECHO_SIGN","1");
    for name in ["Q793_RANK_ECHO_ENTRY","Q793_RANK_ECHO_EXIT"]{set_default_env(name,"1");}
    // Direction-specific measurement cleanup of the inverse R00 Sign producer.
    set_default_env("Q793_MBU_REV_R00","1");
    set_default_env("Q793_T10_GUARD_FACTOR","1");
    set_default_env("Q793_HELD_LOAN","1");
    set_default_env("Q793_FINAL_FLAG","1");
    set_default_env("Q793_FRAME","1");
    // W4 levers (lane-kimi-w4): A2 Sign/newborn gather min-clamp to the
    // analytic T-top support; A4 exit-second selector clamp to the block
    // A-support. Each is isolated by its own kill switch for A/B census.
    set_default_env("Q793_A2_MIN_CLAMP","1");
    set_default_env("Q793_A4_CMAX","1");
    // W5 synthesis (lane-kimi-w45): baked affine-frame winners in
    // metadata_muxlease::swap_plan; =0 restores the W4 bitstream exactly.
    set_default_env("Q793_TABLE_AFFINE","1");
    // A12+A16-T10 on the 9e4cfaf (A11-on-W45) body: T10 v10 digit window.
    // =0 restores unbounded (3..=255) leaves. A18 is HARD_NACK (dirty helper
    // extra term g∧p1∧B); default OFF, opt-in with Q793_A18=1.
    set_default_env("Q793_A16_V10","1");
    ...
    // selectors qualified by the frozen 256-shot run must emit ON by source
    // default. This is a DEFAULT-ONLY change: no arithmetic is altered, no
    // selector is added or removed, and Q793_A18 / Q793_A19_SM0 remain "0"
    // above. set_default_env only fills an UNSET variable, so an explicit
    // environment still overrides, which keeps the diagnostic profile usable.
    ...
    for name in ["Q794_COUNTER_REFLECTION","Q794_COUNTER_AFFINE"]{set_default_env(name,"1");}
    for name in ["Q795_SIGN_TRANSITION","Q796_SIGN_MASK_LOAN","Q795_ENTRY_FACTOR"]{set_default_env(name,"1");}
    // Q795 gate-count release: exact gather sharing and closed decoder caches.
    // Consumers keep their guards; all borrowed cache bits are restored.
    for name in ["Q795_CARGO_PAIR","Q795_R01_GUARD_NARROW","Q795_T11_DIRECT_CARRY","Q795_T10_QUOTIENT_PAIR","Q795_DECODER_DERIVATIVE","Q795_DECODER_FACTOR","Q795_DECODER_UNGUARDED"]{set_default_env(name,"1");}
    set_default_env("Q795_R01_LOW_PREFIX","2");
    set_default_env("Q795_R00_PREFIX","1");
    // Release2: closed-lifetime prefix sharing and paired off-branch maps.
    // Only separately proved paired DATA/basis actions omit their guards.
    for name in ["Q795_C1_PREFIX_TRANSITION","Q795_SIGN_PAIR_UNGUARDED","Q795_R00_PAIR_UNGUARDED","Q795_R00_DATA_CLEAN_EXTENSION","Q795_AUPDATE_PREFIX_TRANSITION","Q795_AUPDATE_PERM_UNGUARDED","Q795_R01_DATA_UNGUARDED","Q795_EXIT_PERM_CLEAN","Q795_R00_PREFIX_TRANSITION","Q795_R01_PREFIX_TRANSITION","Q795_ENTRY_OUTPUT_ERASE"]{set_default_env(name,"1");}
    for (name,value) in [("Q795_C1_ACTIVE_PREFIX","8"),("Q795_R00_CACHE_CLEAN","2"),("Q795_R00_LOWER_BITS","2"),("Q795_R00_UPPER_BITS","3"),("Q795_EXIT_PREFIX","5"),("Q795_AUPDATE_PREFIX","6")]{set_default_env(name,value);}
    // Release3: direct carry comparisons and a paired R01 carry basis.
    for name in ["Q795_R00_TWO_PASS","Q795_SIGN_TWO_PASS","Q795_R01_BASIS_CLEAN_EXTENSION"]{set_default_env(name,"1");}
    // Release4: two-pass ADD with existing loans and closed decoder lifetimes.
    for name in ["Q795_C1_TWO_PASS","Q795_R01_TWO_PASS","Q795_R01_LOWER_CONDITIONAL_SCRATCH","Q795_SIGN_G_SCRATCH_DATA","Q795_SIGN_G_SCRATCH_MASK","Q795_T10_TOP_TWO_PASS"]{set_default_env(name,"1");}
    // Release5: exact T10 arithmetic/quotient fusion and closed cache scratch.
    for name in ["Q795_T10_TOP_MASK_CLEAN","Q795_R01_CACHE_RANK_CLEAN","Q795_R01_CACHE_CARRY_CLEAN","Q795_T10_FUSED"]{set_default_env(name,"1");}
    // Release6: quotient-high-input R01 fusion, with omitted parity preserved.
    set_default_env("Q795_R01_FUSED","1");
    // Own Q794-T R02: native components, all104 steps, full physical
    // lifecycle and independently seeded whole64 passed. Final source-bound
    // canonical9024 and official validation are separate release gates.
    for name in ["Q794_T10_COMBINED_LOAN","Q794_T10_PAIRED_LOW","Q794_T10_ENDPOINT_LOCAL","Q794_T10_ENDPOINT_CACHE","Q794_T10_CACHE_PAIRED","Q794_SIGN_COMBINED_TOP","Q794_SIGN_FACTOR_LOW","Q794_SIGN_PAIRED_LOW","Q794_R01_SEED_FACTOR","Q794_TFACTOR","Q794_AFFINE_ENDPOINT_EXCHANGE","Q794_AFFINE_PREFIX_TRANSITION","Q794_R01_ENDPOINT_FACTOR","Q794_T10_C1_GUARD_FACTOR","Q794_T10_POLISH"] {set_default_env(name,"1");}
    for name in ["Q796_R01_CACHE","Q796_R01_CLEAN","Q796_PREFIX_MASK_LOAN","Q796_DECODER_TRANSITION","Q796_DECODER_ANF","Q796_R00_CACHE","Q796_R00_SUPPORT"]{set_default_env(name,"1");}
    // Source-default binding of the Q802 sequence checked in the full native
    // 9024-shot developer run; the remote trusted benchmark still validates it.
    set_default_env("LOWQ_SHARED_LENGTH", "1");
    set_default_env("LOWQ_SHIFT_PARITY_LOAN", "1");
    set_default_env("LOWQ_METADATA_PARITY_LOAN", "1");
    set_default_env("LOWQ_STEP_OPTIMIZE", "1");
    ...
    if let Ok(mode) = std::env::var("LOWQ_Q793_NATIVE_MODE") {
        match mode.as_str() {
            "r01-timefix-a1-unified" => trailmix_port::inversion::q793_r01_a1_unified_check_timefix_r01::run(),
            "r01-timefix-dynamic" => trailmix_port::inversion::q793_r01_dynamic_timefix_r01::run(),
            "r01-timefix-supported" => trailmix_port::inversion::q793_r01_supported_timefix_r01::run(),
            "r01-numeric-seed" => trailmix_port::inversion::q793_r01_normal_timefix_r01::run_numeric_seed_check(),
            "r01-numeric-scan" => trailmix_port::inversion::q793_r01_normal_timefix_r01::run_numeric_scan_check(),
            "t10-full-v13" => trailmix_port::inversion::q793_t10_full_check_v13::run(),
            "sign-r06" => trailmix_port::inversion::q793_sign_dynamic_r06::verification::run(),
            "step-rank-echo-check" => trailmix_port::inversion::q793_step_r03_check::rank_echo_differential(),
            "c1-hold-check" => trailmix_port::inversion::q793_step_r03_check::c1_hold_differential(),
            "c1-j2-hold-check" => trailmix_port::inversion::q793_step_r03_check::c1_j2_hold_differential(),
            "sign-c12-hold-check" => trailmix_port::inversion::q793_step_r03_check::sign_c12_hold_differential(),
            "runtime-probe" => trailmix_port::inversion::q793_runtime_probe_r01::run(),
            "runtime-check" => q793_runtime_r01::check(),
            "nct-frame-check" => trailmix_port::inversion::q793_nct_frame_r03::check(),
            "whole-default-count" => {
                assert!(trailmix_port::inversion::q793_lifecycle_r03::candidate_configuration() && trailmix_port::inversion::q793_lifecycle_r03::codex10h_resources().is_none(),"source-default Q793 configuration required");
                std::env::set_var("POINT_ADD_COUNT_ONLY","1");
                let b=trailmix_port::build_builder();assert!(b.count_only&&b.ops.is_empty());
                eprintln!("Q793_WHOLE_DEFAULT_COUNT_PASS mode={mode} peak={} ops={}; resource count, not correctness",b.peak_qubits,b.current_ops_len());
            },
            "whole-count" | "whole-stream" => {
                std::env::set_var("LOWQ_Q793_EEA","1");
                std::env::set_var("POINT_ADD_COUNT_ONLY","1");
                if mode=="whole-stream"{
                    std::env::set_var("SPRINT_STREAM_CHECK","1");
                    std::env::set_var("SPRINT_STREAM_SEED","q793-resume-r01-predeclared-independent-seed-v1");
                }
                let b=trailmix_port::build_builder();assert!(b.count_only&&b.ops.is_empty());
                eprintln!("Q793_WHOLE_DIAGNOSTIC_PASS mode={mode} peak={} ops={}; NOT canonical9024 or official acceptance",b.peak_qubits,b.current_ops_len());
            },
            "whole-9024" => {
                assert!(trailmix_port::inversion::q793_lifecycle_r03::candidate_configuration(),"canonical9024 requires a measured Q793 configuration");
                std::env::set_var("LOWQ_WHOLE_COMPACT_SELFTEST","1");
                std::env::remove_var("POINT_ADD_COUNT_ONLY");
                q793_compact_check_r01::run();
            },
            "mod8-cycle" => trailmix_port::inversion::q793_mod8::run(),
            "table-harvest" => trailmix_port::inversion::q793_table_harvest::run(),
            "r01-fused-low" => trailmix_port::inversion::q793_r01_fused_low::run(),
            "r01-short" => trailmix_port::inversion::q793_r01_short::run(),
            "t10-normal" => trailmix_port::inversion::q793_t10_normal_check::run(),
            "t10-normal-v2" => trailmix_port::inversion::q793_t10_normal_check_v2::run(),
            "r01-normal" => trailmix_port::inversion::q793_r01_normal::run(),
            "r01-normal-v2" => trailmix_port::inversion::q793_r01_normal_v2::run(),
            "r01-dynamic-v1" => trailmix_port::inversion::q793_r01_dynamic_v1::run(),
            "r01-dynamic-v2" => trailmix_port::inversion::q793_r01_dynamic_v2::run(),
            "sign-r04" | "sign-r04-count" => trailmix_port::inversion::q793_sign_dynamic_r04::verification::run(),
            "r01-clean-route-core" => trailmix_port::inversion::q793_r01_normal_v4::run(),
            "r01-dynamic-v8" => trailmix_port::inversion::q793_r01_dynamic_v8::run(),
            "r01-supported-v9" => trailmix_port::inversion::q793_r01_supported_v9::run(),
            "sign-r03" | "sign-r03-count" => trailmix_port::inversion::q793_sign_dynamic_r03::verification::run(),
            "r01-a1s1-synth" => trailmix_port::inversion::q793_r01_a1s1_check_v1::run(),
            "step-route-r02" => trailmix_port::inversion::q793_step_route_r02_check::run(),
            "r00-physical-r02" | "r00-physical-r02-all" => trailmix_port::inversion::q793_r00_physical_check_r02::run(),
            "r01-paired-core" => trailmix_port::inversion::q793_r01_normal_v3::run(),
            "r01-dynamic-v6" => trailmix_port::inversion::q793_r01_dynamic_v6::run(),
            "r01-supported-v7" => trailmix_port::inversion::q793_r01_supported_v7::run(),
            "r01-supported-v5" => trailmix_port::inversion::q793_r01_supported_v5::run(),
            "step-route-r03" => trailmix_port::inversion::q793_step_route_r03_check::run(),
            "t10-full-v7" => trailmix_port::inversion::q793_t10_full_check_v7::run(),
            "t10-full-v11" => trailmix_port::inversion::q793_t10_full_check_v11::run(),
            "r01-cflags-core" => trailmix_port::inversion::q793_r01_normal_v5::run(),
            "r01-dynamic-v10" => trailmix_port::inversion::q793_r01_dynamic_v10::run(),
            "r01-supported-v11" => trailmix_port::inversion::q793_r01_supported_v11::run(),
            "t10-full-v10" => trailmix_port::inversion::q793_t10_full_check_v10::run(),
            "sign-r05" | "sign-r05-count" => trailmix_port::inversion::q793_sign_dynamic_r05::verification::run(),
            "r01-clean-routes" => trailmix_port::inversion::q793_r01_routes_check_v1::run(),
            "t10-full-v9" => trailmix_port::inversion::q793_t10_full_check_v9::run(),
            "t10-full-v8" => trailmix_port::inversion::q793_t10_full_check_v8::run(),
            "t-census" => trailmix_port::inversion::q793_lifecycle_r03::sprint_census(),
            "mbu-census" => trailmix_port::inversion::q793_mbu::census(),
            "sign-r01" => trailmix_port::inversion::q793_sign_dynamic_r01::verification::run(),
            "sign-r02" | "sign-r02-count" => trailmix_port::inversion::q793_sign_dynamic_r02::verification::run(),
            "t10-full-v3" => trailmix_port::inversion::q793_t10_full_check_v3::run(),
            "t10-full-v4" => trailmix_port::inversion::q793_t10_full_check_v4::run(),
            "t10-full-v5" => trailmix_port::inversion::q793_t10_full_check_v5::run(),
            "t10-full-v6" => trailmix_port::inversion::q793_t10_full_check_v6::run(),
            "r01-dynamic-v4" => trailmix_port::inversion::q793_r01_dynamic_v4::run(),
            "r01-supported-v3" => trailmix_port::inversion::q793_r01_supported_v3::run(),
            "lifecycle-boundaries" => trailmix_port::inversion::q793_lifecycle_r03::check::run_boundaries(),
            "lifecycle" => trailmix_port::inversion::q793_lifecycle_r03::check::run(),
            ...
            "fourhole-r01diff" => trailmix_port::inversion::q793_lifecycle_r03::diff::run_r01_diff(),
            "fourhole-stepdiff" => trailmix_port::inversion::q793_lifecycle_r03::diff::run_step_diff(),
            "fourhole-stepbisect" => trailmix_port::inversion::q793_lifecycle_r03::diff::run_step_bisect(),
    ...
    if let Ok(mode) = std::env::var("LOWQ_Q794_NATIVE_MODE") {
        match mode.as_str() {
            "t-census" => trailmix_port::inversion::q794_lifecycle::sprint_census(),
            "dual-exit" | "dual-exit-offguard" => trailmix_port::inversion::q794_exit_check::run(),
            "r00" | "r00-all" => trailmix_port::inversion::q794_r00_check::run(),
            "r01-fused-low" => trailmix_port::inversion::q794_r01_fused_low::run(),
            "t10-fused-low" => trailmix_port::inversion::q794_t10_fused_low::run(),
            "cargo-step" => trailmix_port::inversion::q794_cargo_step_check::run(),
            "r01-dynamic" => trailmix_port::inversion::q794_r01::run(),
            "r01-seed-factor" => trailmix_port::inversion::q794_r01::run_seed_factor(),
            "t10-dynamic" => trailmix_port::inversion::q794_t10_check::run(),
            "sign-dynamic" => trailmix_port::inversion::q794_sign::verification::run(),
            "sign-general" => trailmix_port::inversion::q794_sign_general_check::run(),
            "full-step" => trailmix_port::inversion::q794_step_check::run(),
            "lifecycle" => trailmix_port::inversion::q794_lifecycle::check::run(),
            "lifecycle-boundaries" => trailmix_port::inversion::q794_lifecycle::check::run_boundaries(),
            "whole-9024" => {
                std::env::set_var("LOWQ_Q794_EEA","1");
                std::env::set_var("Q794_MOD4","1");
                std::env::set_var("LOWQ_WHOLE_COMPACT_SELFTEST","1");
                std::env::remove_var("POINT_ADD_COUNT_ONLY");
                q794_compact_check::run();
            },
            "whole-count" | "whole-stream" | "whole-default-count" | "whole-experimental-count" | "whole-experimental-stream" => {
                if mode!="whole-default-count"{std::env::set_var("LOWQ_Q794_EEA","1");}
                if mode.starts_with("whole-experimental-") {
                    assert_eq!(std::env::var("Q794_EXPERIMENTAL").ok().as_deref(),Some("1"));
                } else {assert!(trailmix_port::inversion::q794_lifecycle::candidate_configuration(),"source-default Q794 configuration required");}
                std::env::set_var("Q794_MOD4","1");
                std::env::set_var("POINT_ADD_COUNT_ONLY","1");
                if mode=="whole-stream" || mode=="whole-experimental-stream"{
                    std::env::set_var("SPRINT_STREAM_CHECK","1");
                    std::env::set_var("SPRINT_STREAM_SEED","q794-treduce-r01-predeclared-independent-seed-v1");
                }
                let b=trailmix_port::build_builder();assert!(b.count_only&&b.ops.is_empty());
                eprintln!("Q794_WHOLE_DIAGNOSTIC_PASS mode={mode} peak={} ops={}; NOT official9024",b.peak_qubits,b.current_ops_len());
            },
            _ => panic!("unknown own Q794 native test: {mode}"),
        }
        std::process::exit(0);
    }
    if std::env::var("LOWQ_RANK5_WHOLE_COUNT_SELFTEST").ok().as_deref()==Some("1") {
        std::env::set_var("POINT_ADD_COUNT_ONLY","1");std::env::set_var("LOWQ_RANK5_EEA","1");
        let b=trailmix_port::build_builder();assert!(b.count_only);assert!(b.ops.is_empty());
        std::process::exit(0);
    }
    if std::env::var("LOWQ_WHOLE_COMPACT_SELFTEST").ok().as_deref()==Some("1") {
        compact_check::run();
        // Exit before the trusted builder can serialize an empty diagnostic artifact.
        std::process::exit(0);
    }
