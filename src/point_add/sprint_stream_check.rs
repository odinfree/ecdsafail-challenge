...
impl Batch {
    ...
    pub fn apply(&mut self,ops:&[Op]) {
        ...
        // Dev diagnostic: dump the first 500 R/Hmr ops of the stream so a
        // correct build and a suspect build can be diffed by offset.
        if std::env::var("Q792_GHOST_DUMP").ok().as_deref()==Some("1") {
            ...
            for op in ops {
                ...
                if matches!(op.kind, K::R | K::Hmr) {
                    if SEEN.fetch_add(1, std::sync::atomic::Ordering::Relaxed) >= 500 { continue; }
                    eprintln!(
