...
pub fn mod_mul_canonical_mbu(circ: &mut Circuit, result: &[QReg], a: &[QReg], b: &[QReg]) {
    ...
    let lender=if crate::point_add::trailmix_port::inversion::q796_parity::enabled()&&a.iter().chain(result).all(|q|q.id()!=b[256].id()){Some(&b[256])}else{None};
    ...
    crate::point_add::trailmix_port::mod_arith::controlled_mod_add_canonical_mbu_with_lender(
        circ, &b[255], result, a,lender,
    );
    for i in (0..255).rev() {
        crate::point_add::trailmix_port::mod_arith::mod_double_canonical_mbu_with_lender(circ, result,lender);
        crate::point_add::trailmix_port::mod_arith::controlled_mod_add_canonical_mbu_with_lender(
            circ, &b[i], result, a,lender,
        );
    }
...
pub fn mod_mul_canonical_mbu_undo(
    ...
    // b[256] is clean and never used as a Horner control. Preserve the old
    // route when inputs alias; otherwise lend that existing lane to the
    // subtraction's exact phase correction and restore it after each cell.
    let phase_lender=if a.iter().chain(result).all(|q|q.id()!=b[256].id()){Some(&b[256])}else{None};
    let dual=crate::point_add::trailmix_port::inversion::q796_parity::enabled()&&phase_lender.is_some();
    let borrow_lender=if dual{Some(&b[256])}else{None};
    let phase_lender=if dual{Some(&a[256])}else{phase_lender};

    for i in 0..255 {
        crate::point_add::trailmix_port::mod_arith::controlled_mod_sub_canonical_mbu_with_loans(
            circ, &b[i], result, a,borrow_lender,phase_lender,
        );
        crate::point_add::trailmix_port::mod_arith::mod_halve_canonical_mbu(circ, result);
    }
    crate::point_add::trailmix_port::mod_arith::controlled_mod_sub_canonical_mbu_with_loans(
        circ, &b[255], result, a,borrow_lender,phase_lender,
    );
