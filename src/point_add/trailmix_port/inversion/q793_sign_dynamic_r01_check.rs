...
pub fn run(){
    ...
    for j in 0..4{
        ...
        eprintln!("Q793_SIGN_R01_BUILT j={j} rows={} ops={} T={}",rows.len(),builder.ops.len(),builder.ops.iter().filter(|o|o.kind==K::CCX).count());
        for pattern in 0..11{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let bad:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();
                let lanes:u64=sim.qubits.iter().zip(&after).map(|(x,y)|x^y).fold(0,|x,y|x|y);let cases:Vec<_>=(0..64).filter(|&l|lanes>>l&1!=0).map(|l|{let r=rows[(64*batch+l)%rows.len()];(l,r[128],r[129],r[130],r[131])}).collect();panic!("Sign r01 j={j} pattern={pattern} batch={batch} diffs={bad:?} cases={cases:?}");}
            assert_eq!(sim.phase,0);sim.apply_iter(builder.ops.iter().rev());assert_eq!(sim.qubits,before);assert_eq!(sim.phase,0);total+=64;
        }}
    }
    eprintln!("Q793_SIGN_R01_PASS lanes={total} active={active} short={short} S0={s0} A254={a254} scalar_rows={rowcount}; complete Sign wrapper, one funded mask, no clean carry, all three absent rails, all eight cargo patterns and three off phases, exact phase/inverse/allwire restoration; no whole-step claim");
}
