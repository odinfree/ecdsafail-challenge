...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        emit(&mut circ,&rank,&a,&c,&sm,&g,&mask,&hs,&ha,&decision,&w1,&w2,&dirty,j,259);
        assert_eq!(circ.b.next_qubit,nq);let ops=circ.b.ops.clone();for op in &ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        let nt=ops.iter().filter(|o|o.kind==K::CCX).count();eprintln!("Q793_R01_NORMAL_BUILT j={j} ops={} T={nt}",ops.len());
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|row|{
            ...
            (time+3)&3==j&&bits(r,21,2)==2&&av>=2&&av+cv<=253
        }).collect();
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (a,b))|a!=b).map(|(i,(a,b))|(i,format!("{:016x}",a^b))).collect();panic!("Q793 R01 normal j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        for batch in 0..32{let mut seed=0x793f010ffu64^((j as u64)<<40)^batch as u64;let mut before:Vec<_>=(0..nq).map(|_|rnd(&mut seed)).collect();
            for lane in 0..64{set(&mut before,&rank,lane,batch);set(&mut before,&a,lane,lane);put(&mut before,&g,lane,false);}
        ...
        rows_out.push(format!("{{\"clock\":{j},\"active_lanes\":{here},\"T\":{nt},\"ops\":{}}}",ops.len()));
    }
    println!("{{\"kind\":\"Q793 normal R01 external-zero-lease dynamic core\",\"active_lanes\":{total},\"offguard_lanes\":{off},\"shift0_1_2_ge3\":{counts:?},\"A2\":{a2},\"A252_253\":{high_a},\"results\":[{}],\"zero_lease_wrappers_tested\":false,\"whole_q793\":false}}",rows_out.join(","));
    eprintln!("Q793_R01_NORMAL_PASS active={total} offguard={off}; native phase/inverse,three physical holes, low3 qpre/borrow, A2head/two-passenger/highA, external leases only; no wholeQ793 claim");
}
