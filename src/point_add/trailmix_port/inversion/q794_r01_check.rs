...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        let p1=circ.alloc_qreg("p1");let p2=circ.alloc_qreg("p2");let w1=circ.alloc_qreg_bits("w1",259);let w2=circ.alloc_qreg_bits("w2",259);let dirty=circ.alloc_qreg_bits("borrowed",24);let owned=circ.b.next_qubit;
        super::signless(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&w1,&w2,&dirty,j,259);assert_eq!(circ.b.next_qubit,owned);
        let builder=circ.into_builder();for op in &builder.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        let nt=builder.ops.iter().filter(|op|op.kind==K::CCX).count();eprintln!("Q794_R01_BUILT j={j} ops={} T={nt} interfaceQ={owned}",builder.ops.len());
        ...
        for pattern in 0..4{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                if !t0{for bit in 0..2{put(&mut before,&w2[(259+bit-sv%259)%259],lane,bits(r,25+258-bit,1)!=0);}}
                ...
                assert!(!val(&before,&w1[av+1],lane));assert!(!val(&before,&w2[av+1],lane));
                // Holes are deliberately arbitrary and must stay untouched.
                put(&mut before,&w1[257],lane,rnd(&mut seed)&1!=0);put(&mut before,&w1[258],lane,rnd(&mut seed)&1!=0);
                for i in 0..owned as usize{after[i]=(after[i]&!(1u64<<lane))|(before[i]&(1u64<<lane));}
                if sum==255{
                    assert_eq!(shift,0);assert_eq!(bits(r,25+257,1),0);assert_eq!(bits(r,284+258-sv,1),1);
                    if !t0{let b0=val(&before,&w2[0],lane);assert!(!val(&before,&w2[1],lane));put(&mut after,&w2[0],lane,false);put(&mut after,&w2[1],lane,b0);}
                    endpoint+=1;
                }else{
                    let n=256-sum;assert!(n>=2);let mut x=vec![false;n];let mut y=vec![false;n];
                    ...
                    for i in 2..n{put(&mut after,&w1[258-i],lane,out[i]);}put(&mut after,&w1[sum+2],lane,q);
                    if !t0{for bit in 0..2{put(&mut after,&w2[(259+bit-shift)%259],lane,out[bit]);}}
                }
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q794_R01 active j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        for phase in [0usize,1,3]{for batch in 0..32{
            ...
            for lane in 0..64{let rk=batch;let av=64*ts[rk][0]+lane;set(&mut before,&rank,lane,rk);set(&mut before,&a,lane,lane);put(&mut before,&p1,lane,phase&1!=0);put(&mut before,&p2,lane,phase&2!=0);put(&mut before,&w1[av+1],lane,false);}
        ...
        eprintln!("Q794_R01_CLOCK_PASS j={j} active={active_here}");
    }
    println!("{{\"kind\":\"candidate actual dynamic dual-cargo R01\",\"active_lanes\":{total},\"offguard_lanes\":{off},\"A0\":{a0},\"A1\":{a1},\"A254_S1_v1_cargo\":{cargo_v1},\"overflow_endpoint\":{endpoint},\"results\":[{}],\"whole_q794_proven\":false}}",results.join(","));
    eprintln!("Q794_R01_PASS active={total} offguard={off}; actual rank5 ranges, A0/A1, two independent cargo bits, q1 gather, omitted low2, width2 overflow, HA restored, literal inverse/allphase; no whole-circuit claim");
}
