...
pub fn run(){
    ...
    for block in 0..26{for j in 0..4{
        ...
        super::q793_step_route_r03::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&it,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,565);let b=circ.into_builder();for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        ...
        eprintln!("Q793_STEP_ROUTE_R03_BLOCK block={block} j={j} ops={} T={t} physical=565 PASS",b.ops.len());
    }}
    eprintln!("Q793_STEP_ROUTE_R03_PASS lanes={lanes} forward_ops={weighted_ops} forward_T={weighted_t}; own integrated three-hole/mod8 step; holes256/257/258 absent; NOT whole point-add Q793 or9024");
}
