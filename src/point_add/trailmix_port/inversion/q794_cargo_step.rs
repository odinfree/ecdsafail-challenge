//! Own routing-only Q794 precursor: inherited arithmetic still keeps r1.
...
fn newborn(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p2:&QReg,sign:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg],j:usize){
    ...
    super::q794_cargo_extreme::newborn(circ,rank,a,sign,left,right,dirty);
...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    super::q795_t10::emit(circ,rank,a,c,p1,p2,w1,w2,helpers,tend);
    mark(circ,"T10");
    ...
    super::q794_cargo_extreme::inbound(circ,rank,a,w1,w2,2,&tflag,dirty);
    toggle_terms(circ,&c1_t10,sign,dirty);
    super::q794_cargo_extreme::before_entry(circ,rank,a,c,sm,p1,p2,w1,w2,dirty);
    ...
    super::q794_cargo_extreme::after_entry(circ,rank,a,sign,w1,w2,dirty);
    mark(circ,"entry");
    ...
    if !to_c1.is_empty(){toggle_terms(circ,&to_c1,sign,dirty);super::q794_cargo_extreme::inbound(circ,rank,a,w1,w2,0,&rflag,dirty);toggle_terms(circ,&to_c1,sign,dirty);}
    ...
    moves::across_a_terms(circ,rank,a,w2,2,w1,2,&c1,true,&pool);
    let window=std::env::var("Q795_CORE_CANCEL").ok().map(|s|s.parse::<usize>().unwrap()).unwrap_or(2048);
    mark(circ,"final");
    if std::env::var_os("Q795_TRACE").is_none(){let mut tail=circ.b.ops.split_off(start);super::shared_optimize::cancel_nct(&mut tail,window,8);super::shared_optimize::cancel_nct_live(&mut tail,window);circ.b.ops.extend(tail);}circ.q797_a_support=previous;
}
fn birth_cargo(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],dirty:&[QReg]){
    ...
    moves::adjacent_a_terms(circ,rank,a,w1,2,3,&vec![term],dirty);
}
pub(super) fn r01_correction(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],g:&QReg,w1:&[QReg],w2:&[QReg],dirty:&[QReg]){
    ...
    let (source,s)=super::q798_handoffs::gather_a(circ,rank,a,w2,2,dirty);
    let (target,t)=super::q798_handoffs::gather_a(circ,rank,a,w1,2,dirty);
