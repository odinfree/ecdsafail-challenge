...
/// Q793_CARGO_A_SUPPORT: prune A=253/254 cargo monomials in blocks whose
/// A-support has hi<=253 (A<hi<=253, so A<=252 and those A values are
/// unreachable). A=255 is kept: it is the sentinel value and is never pruned.
fn a_pruned(circ:&Circuit,value:usize)->bool{
    super::metadata_muxlease::active("Q793_CARGO_A_SUPPORT")
        && value!=255
        && circ.q797_a_support.map_or(false,|(_,hi)|hi<=253)
}
...
fn below253<'a>(circ:&Circuit,rank:&'a[QReg],a:&'a[QReg],terms:&Terms<'a>)->Terms<'a> {
    let mut out=terms.clone();for value in 253..=255{if !a_pruned(circ,value){out.extend(at(circ,rank,a,value,terms));}}out
}
...
pub(super) fn before_entry(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w2:&[QReg],dirty:&[QReg]) {
    ...
    let terms=at(circ,rank,a,254,&vec![base]);
...
pub(super) fn after_entry(circ:&mut Circuit,rank:&[QReg],a:&[QReg],sign:&QReg,w2:&[QReg],dirty:&[QReg]) {
    let terms=at(circ,rank,a,254,&vec![vec![(sign,true)]]);
...
pub(super) fn newborn_253(circ:&mut Circuit,rank:&[QReg],a:&[QReg],sign:&QReg,left:&QReg,right:&QReg,dirty:&[QReg]) {
    let terms=at(circ,rank,a,253,&vec![vec![(sign,true)]]);
    swap(circ,&terms,left,right,dirty);flip(circ,&terms,left,dirty);
}

/// Finish C1 transport. A253 was already exchanged before entry.
/// At A254 the destination W2[255] has base0, unlike omitted W1[256]
...
pub(super) fn finish_c1(circ:&mut Circuit,rank:&[QReg],a:&[QReg],w1:&[QReg],w2:&[QReg],terms:&Terms<'_>,dirty:&[QReg]) {
    ...
    let cs=at(circ,rank,a,254,terms);swap(circ,&cs,&w2[256],&w2[255],dirty);
}
...
pub(super) fn normalize_old11(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w2:&[QReg],j:usize,dirty:&[QReg]) {
    let mut terms=at(circ,rank,a,254,&vec![vec![(p1,true),(p2,true)]]);
    if j==0 {
