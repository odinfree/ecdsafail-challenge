...
pub(super) fn run(){
    ...
    for j in 0..4{
        ...
        let nt=builder.ops.iter().filter(|op|op.kind==K::CCX).count();eprintln!("Q793_DYNAMIC_V10_R01_BUILT j={j} ops={} T={nt} interfaceQ={owned}",builder.ops.len());
        ...
        for pattern in 0..4{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q793_DYNAMIC_V10_R01 active j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
        ...
        eprintln!("Q793_DYNAMIC_V10_R01_CLOCK_PASS j={j} active={active_here}");
    }
    ...
    eprintln!("Q793_DYNAMIC_V10_R01_PASS active={total} offguard={off}; actual rank5 ranges, A0/A1, two independent cargo bits, q1/q2 gather, three physical holes, packed width2/1 endpoints, constant-S1 HA restored, literal inverse/allphase; conditional SM3 lifetime and physical donors; no whole-circuit claim");
}
