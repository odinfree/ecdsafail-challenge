...
pub fn run(){
    ...
    for block in 0..26{for j in 0..4{
        ...
        super::q794_cargo_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&it,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,565);let b=circ.into_builder();for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        ...
        eprintln!("Q794_DUAL_CARGO_STEP_BLOCK block={block} j={j} ops={} T={t} physical=565 PASS",b.ops.len());
    }}
    eprintln!("Q794_DUAL_CARGO_STEP_PASS lanes={lanes} forward_ops={weighted_ops} forward_T={weighted_t}; own dual-cargo routing only; r1 still retained; NOT Q794 or full point-add");
}
