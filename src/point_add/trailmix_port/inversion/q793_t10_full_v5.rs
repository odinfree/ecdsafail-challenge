...
use super::q793_t10_expand_v5::ceq;
...
fn branch(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],n:usize,j:usize,last:bool,low:bool){
    ...
    if low{super::q793_t10_expand_v5::flags(circ,rank,a,c,g,&sm[1],&sm[2],dirty);super::q793_t10_expand_v5::emit(circ,rank,a,c,sm,g,w1,w2,dirty,j,false);}
    ...
    move_g(circ,rank,a,g,&source,w2,dirty);
    if low{super::q793_t10_expand_v5::emit(circ,rank,a,c,sm,g,w1,w2,dirty,j,true);super::q793_t10_expand_v5::flags(circ,rank,a,c,g,&sm[1],&sm[2],dirty);}
    guard(circ,rank,a,c,sm,p1,p2,g,dirty,j,last,low);
    super::q793_loans::global_a(circ,rank,a,g,w1,&w2[258],None,dirty);
}
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],n:usize,j:usize){
    ...
    for op in &circ.b.ops[start..]{for h in [256,257,258]{let q=w1[h].id()as u64;assert!(op.q_target.0!=q&&op.q_control1.0!=q&&op.q_control2.0!=q,"T10 V5 touched omitted W1[{h}]");}}
