...
fn length_xor(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],out:&[&QReg],source:&[QReg],prefix:&[QReg],guard:&QReg,helpers:&[QReg],lo:usize,hi:usize,decode_u:bool) {
    let chart=[&prefix[0],&prefix[1],&source[0],&source[1],&source[258],&source[257]];
    ...
    let protected:Vec<_>=prefix.iter().enumerate().map(|(i,q)|if decode_u&&i<2{helpers[i].borrowed_alias()}else{q.borrowed_alias()}).collect();
    let prefix=&protected;
    let helpers=if decode_u{assert!(helpers.len()>=18);&helpers[2..]}else{helpers};
    ...
    let cell=|circ:&mut Circuit,i:usize| {
        ...
        if hi-1-i==0&&super::q796_parity::enabled(){mixed_mcx(circ,&[(parent,true),(mask,true)],&prefix[i],helpers);}else if hi-1-i==1&&decode_u{super::q794_exit_low::xor_u1(circ,chart,&[(parent,true),(mask,true)],&prefix[i],helpers);}else{mixed_mcx(circ,&[(parent,true),(&source[hi-1-i],true),(mask,true)],&prefix[i],helpers);}
    };
