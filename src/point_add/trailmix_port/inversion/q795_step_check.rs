...
pub fn run(){
    ...
    for block in 0..26{for j in 0..4{
        ...
        super::q795_step::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&it,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,565);let b=circ.into_builder();for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));}
        ...
        assert_eq!(touches,0,"Q795 must never address the omitted parity rail");
        ...
        for pattern in 0..4 {for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{let row=rows[(batch*64+lane)%rows.len()];let cargo=match pattern{0=>false,1=>true,_=>rnd(&mut seed)&1!=0};
                let second=match pattern{0=>true,1=>false,_=>rnd(&mut seed)&1!=0};
            ...
            if sim.qubits!=after{
                if std::env::var_os("Q795_TRACE").is_some(){
                    ...
                    for(name,to)in super::q795_step::marks(){sim.apply_iter(b.ops[from..to].iter());from=to;let val=|first:usize,n:usize|->usize{(0..n).map(|i|(((sim.qubits[first+i]>>lane)&1)as usize)<<i).sum()};let r=val(0,5);eprintln!("DUAL_TRACE stage={name} ops={to} lane={lane} A={} C={} sm={} phase={} iter={} w1low={} w2low={} sign={}",64*ts[r][0]+val(5,6),64*ts[r][1]+val(11,6),val(17,4),val(21,2),val(23,1),val(24,12),val(283,12),val(542,1));}
                }
    ...
    eprintln!("Q795_DUAL_STEP_PASS lanes={lanes} forward_ops={weighted_ops} forward_T={weighted_t}; native encoded transitions only, hole/allocator/lifecycle/whole verification separate");
}
