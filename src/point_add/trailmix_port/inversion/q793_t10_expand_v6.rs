//! V6: C1/C>=2 static cofactors and SM0-funded conditional-clean lowering.
...
fn gates(circ:&mut Circuit,p:Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg){for cs in tidy(p){
    let mut others=Vec::new();let mut has_g=false;for(q,v)in cs{if q.id()==g.id(){assert!(v);has_g=true;}else{others.push((q,v));}}
    assert!(has_g);super::conditional_mcx::guarded(circ,g,&others,target,scratch,false,&dirty[0]);
}}
fn anf(circ:&mut Circuit,vars:&[Poly<'_>],f:impl Fn(usize)->bool,base:&Poly<'_>,target:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg){let mut values:Vec<_>=(0..1usize<<vars.len()).map(f).collect();for b in 0..vars.len(){for m in 0..values.len(){if m>>b&1!=0{values[m]^=values[m^(1<<b)];}}}let mut out=Vec::new();for(m,on)in values.into_iter().enumerate(){if on{let mut p=base.clone();for(i,v)in vars.iter().enumerate(){if m>>i&1!=0{p=mul(&p,v);}}out.extend(p);}}gates(circ,out,target,dirty,g,scratch);}
fn swap(circ:&mut Circuit,cond:Poly<'_>,x:&QReg,y:&QReg,dirty:&[QReg],g:&QReg,scratch:&QReg){circ.cx(y,x);gates(circ,mul(&cond,&wire(x)),y,dirty,g,scratch);circ.cx(y,x);}
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool,last:bool){
    assert!(helpers.len()>=22);let start=circ.b.ops.len();let h=&sm[3];let q=&helpers[..3];let dirty=&helpers[3..];let scratch=&sm[0];
    let cc:Vec<_>=(0..=3).map(|i|if last{if i==1{one()}else{Vec::new()}}else if i<=1{Vec::new()}else{ceq(rank,c,i)}).collect();let aa0=aeq(rank,a,0);let aa1=aeq(rank,a,1);
    ...
    swap(circ,mul(&wire(g),&specs),h,&w2[255],dirty,g,scratch);
    ...
    if !last{for k in 0..3{let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v4::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs);}}
    let tt=[wire(&w1[0]),replace(wire(&w1[1]),aa0.clone(),false),replace(wire(&w1[2]),xor(aa0,aa1),false)];
    let qq=[replace(wire(&q[0]),cc[1].clone(),!inverse),replace(replace(wire(&q[1]),cc[1].clone(),false),cc[2].clone(),true),replace(replace(wire(&q[2]),xor(cc[1].clone(),cc[2].clone()),false),cc[3].clone(),true)];
    for shift in 0..=2{
        ...
        for rwidth in [3usize,2,1]{
            ...
            gates(circ,mul(&even,&bb[2]),h,dirty,g,scratch);
            ...
            anf(circ,&vars,|x|{let t=x&7;let r=x>>3&3;let v=x>>5&7;let qs=x>>8&3;(v.wrapping_mul(7usize.wrapping_sub(t*r)).wrapping_sub((qs<<shift)*t))>>2&1!=0},&all_even,b2,dirty,g,scratch);
        }
    }
    if !last{for k in(0..3).rev(){let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v4::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs);}}
