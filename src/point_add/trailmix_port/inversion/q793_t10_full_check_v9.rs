...
pub fn run(){
    ...
    for block in 0..26{for j in 0..4{
        ...
        super::q793_t10_full_v9::emit(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&w1,&w2,&helpers,n,j);assert_eq!(circ.b.next_qubit,565);let b=circ.into_builder();
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q793 full T10 V9 block{block} j{j} pattern{pattern} batch{batch} diffs={diffs:?}");}
        ...
        eprintln!("Q793_T10_FULL_V9_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());
    }}
    eprintln!("Q793_T10_FULL_V9_PASS lanes={total} active={active} C1={c1_count} M255={m255} M256={m256} shortA={short} A253+={extreme} terminal={terminal} C256={c256} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; all saved geometries/all4passengerpairs+2random/packed3holes/dirty/phase/literalinverse; NOT fullstep or wholeQ793");
}
