...
fn template(block:usize,j:usize)->Vec<Op> {
    ...
    assert!(dual_phase());super::q793_step_r01::step(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&iter,&w1,&w2,&helpers,j,block);assert_eq!(circ.b.next_qubit,565);
...
pub fn sprint_census(){
 ...
 for block in 0..26 {if only.is_some_and(|b|b!=block){continue;}
  for j in 0..4 {let ops=template(block,j);let t=ops.iter().filter(|o|o.kind==OperationType::CCX).count();let copies=if block==25{4}else{16};total_ops+=copies*ops.len();total_t+=copies*t;eprintln!("Q793_TEMPLATE block={block} j={j} ops={} T={t} copies={copies}",ops.len());if sample{compressed+=copies as u64*compressed_sample(&ops);}}
 }
...
// Exact count from own stage15 integrated stream. Canonical9024 and
// official validation remain separate release requirements.
pub(crate) const CANDIDATE_OPS:usize=2_211_841_407;
pub(crate) const CANDIDATE_TOFFOLI:usize=1_356_134_756;
pub(crate) fn candidate_configuration()->bool{
    enabled() && super::q794_lifecycle::candidate_configuration()
}
