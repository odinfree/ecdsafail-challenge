...
fn route<'a>(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],w1:&'a[QReg],helpers:&[QReg],offset:isize)->(&'a QReg,Vec<crate::circuit::Op>){
    assert!(helpers.len()>=18&&(-1..=1).contains(&offset));let start=circ.b.ops.len();add(circ,a,c,None,false);let d=&helpers[0];let dirty=&helpers[1..];
...
/// Exact dirty exchange with a retained quotient digit. Logical callers
/// cofactor head/cargo digits and never consume excluded t[0..3) leaves.
pub(super) fn digit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],w1:&[QReg],out:&QReg,helpers:&[QReg],offset:isize){
    let(root,ops)=route(circ,rank,a,c,w1,helpers,offset);
    circ.cx(root,out);circ.cx(out,root);circ.cx(root,out);
    circ.b.ops.extend(ops.into_iter().rev());
}
/// High-S quotient pop/mask: M<=253 and physical addresses M+1<=254.
/// Excluded tiny addresses are used only by known-one C1, handled separately.
