...
fn run_inner(boundary_only:bool){
 ...
 let peak=794;assert!(dual_phase());assert_eq!(circ.b.active_qubits,513);assert_eq!(circ.b.peak_qubits,peak);assert_eq!(circ.b.next_qubit,peak);let owned=circ.b.next_qubit as usize;let bits=circ.b.next_bit as usize;
 eprintln!("Q794_LIFECYCLE_BUILT inputs={count} physical={owned} peak={} bits={bits} init_ops={} release_ops={} to_inverse_ops={} from_inverse_ops={} rebuild_ops={} finish_ops={}",circ.b.peak_qubits,initialization.len(),release.len(),to_inverse.len(),from_inverse.len(),rebuild.len(),finishing.len());
 ...
 for inverse in [false,true]{let mapping=if inverse{&rebuilt_map}else{&initial_map};
  if !boundary_only{for z in 0..26{let block=if inverse{25-z}else{z};let templates:Vec<_>=(0..4).map(|j|remap(template(block,j),mapping,&passenger,inverse)).collect();let first=block*64;let end=(first+64).min(1616);
   ...
   for(batch,sim)in sims.iter().enumerate(){for q in &passenger[if dual_phase(){2}else{1}..]{assert_eq!(sim.qubits[q.id()as usize],initial[batch][q.id()as usize],"passenger inverse={inverse} block={block}");}}eprintln!("Q794_LIFECYCLE_BLOCK inverse={inverse} block={block} PASS");
  }
 ...
 let marker=if boundary_only{"Q794_LIFECYCLE_BOUNDARIES_PASS"}else{"Q794_LIFECYCLE_PASS"};
