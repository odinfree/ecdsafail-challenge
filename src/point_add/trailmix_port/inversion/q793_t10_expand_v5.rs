//! V5: consumed C1 q0 is0 in the post-arithmetic codec,1 before.
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool){
    ...
    for k in 0..3{let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v4::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs);}
    ...
    for k in(0..3).rev(){let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v4::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs);}
