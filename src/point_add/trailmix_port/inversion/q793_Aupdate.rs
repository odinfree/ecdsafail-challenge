...
fn length_xor(circ:&mut Circuit,rank:&[QReg],c:&[QReg],sm:&[QReg],out:&[&QReg],source:&[QReg],prefix:&[QReg],guard:&QReg,helpers:&[QReg],lo:usize,hi:usize,decode_u:bool) {
    ...
    // The mod-16 chart requires its fourth v-rail (v3) to be the CONSTANT ZERO
    // that the four-hole geometry trades away. Reading `source[255]`/`prefix[255]`
    // instead feeds a non-zero rail into the ANF and corrupts every recovery for
    // every A value (the measured 64/64 value failure). Use a scratch lane that
    // is zeroed before the chart runs; the primitive contract only requires the
    // rails to be restored, so a |0> lane is the correct input.
