...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    super::q793_t10_full_v11::emit(circ,rank,a,c,sm,p1,p2,w1,w2,helpers,tend,entry_j);
    mark(circ,"T10");
    super::metadata_rotation5::rotate(circ,rank,a,p1,p2,w2,&pool,false);
    let p01=vec![(p1,false),(p2,true)];
    super::q793_r01_dynamic_v10::signless(circ,rank,a,c,sm,p1,p2,w1,w2,&pool,entry_j,259-rfirst);
    mark(circ,"R01");
    ...
    super::q793_sign_dynamic_r05::emit(circ,rank,a,c,sm,p1,p2,sign,w1,w2,dirty,post_j,(tend+1).min(258));old::hide_birth(circ,rank,a,c,sm,p1,p2,dirty,post_j);super::q793_loans::global_a(circ,rank,a,sign,w1,&w2[258],Some(&w2[257]),None,dirty);
    ...
    if std::env::var_os("Q795_TRACE").is_none(){let mut tail=circ.b.ops.split_off(start);super::shared_optimize::cancel_nct(&mut tail,window,8);super::shared_optimize::cancel_nct_live(&mut tail,window);
        ...
        let (saved_ops,saved_t)=super::q793_nct_frame_r03::apply(&mut tail);if std::env::var_os("Q793_FRAME_CENSUS").is_some(){eprintln!("Q793_FRAME_COST block={block} j={post_j} saved_ops={saved_ops} saved_T={saved_t} remaining_ops={}",tail.len());}circ.b.ops.extend(tail);}circ.q797_a_support=previous;
}
