...
pub(super) fn step(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,iteration:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],post_j:usize,block:usize){
    ...
    mark(circ,"mbu_before_loan");if std::env::var("Q793_HELD_LOAN").ok().as_deref()!=Some("1"){super::q793_loans::global_a(circ,rank,a,sign,w1,&w2[258],None,dirty);}mark(circ,"mbu_after_loan");circ.ccx(p1,p2,sign);
    ...
    super::q793_sign_dynamic_r06::emit(circ,rank,a,c,sm,p1,p2,sign,w1,w2,dirty,post_j,(tend+1).min(258));old::hide_birth(circ,rank,a,c,sm,p1,p2,dirty,post_j);if std::env::var("Q793_HELD_LOAN").ok().as_deref()!=Some("1"){super::q793_loans::global_a(circ,rank,a,sign,w1,&w2[258],None,dirty);}
