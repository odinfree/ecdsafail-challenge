//! Actual new scheduled T10 only, against a separate scalar ADD/pop oracle.
//! Every donor capsule input is retained; no reference circuit is executed.
...
pub fn run(){
    ...
    for block in 0..26{for j in 0..4{
        ...
        super::q794_t10::emit(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&w1,&w2,&helpers,n,j);assert_eq!(circ.b.next_qubit,565);let b=circ.into_builder();
        for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));for i in [257,258]{let q=w1[i].id()as u64;assert!(op.q_target.0!=q&&op.q_control1.0!=q&&op.q_control2.0!=q,"T10 touches omitted W1[{i}]");}}
        ...
        let rows:Vec<_>=data[12..].chunks_exact(138).filter(|r|{let time=u16::from_le_bytes(r[..2].try_into().unwrap())as usize;(time-1)/64==block&&(time-1)%4==j}).collect();assert!(!rows.is_empty());
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            for lane in 0..64{
                ...
                if !on{for w in [&mut before,&mut after]{for h in [257,258]{put(w,&w1[h],lane,false);}}continue;}
                ...
                for (w,output)in [(&mut before,false),(&mut after,true)]{
                    ...
                    if even{
                        ...
                        else{let r0=get(w,&w1[258],lane);let r1=get(w,&w1[257],lane);put(w,&w2[b0],lane,r0);put(w,&w2[b1],lane,r1);}
                    ...
                    for h in [257,258]{put(w,&w1[h],lane,false);}
                }
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (x,y))|x!=y).map(|(i,(x,y))|(i,format!("{:016x}",x^y))).collect();panic!("Q794 T10 block{block} j{j} pattern{pattern} batch{batch} diffs={diffs:?}");}
            assert_eq!(sim.phase,0);sim.apply_iter(b.ops.iter().rev());assert_eq!(sim.qubits,before);assert_eq!(sim.phase,0);total+=64;
        }}
        eprintln!("Q794_T10_DUAL_BLOCK block={block} j={j} ops={} T={t} PASS",b.ops.len());
    }}
    eprintln!("Q794_T10_DUAL_PASS lanes={total} active={active} C1={c1_count} M256={endpoint} C1A254={extreme} weighted_forward_ops={weighted_ops} weighted_forward_T={weighted_t}; actualnewadapter/allguards/all4passengerpairs+2random/hole257+258/dirty/phase/literalinverse; NOT full step or wholeQ794");
}
