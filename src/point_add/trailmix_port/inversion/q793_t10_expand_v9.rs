//! V9: use the still-zero SM0 as actual quotient-address carry.
...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],g:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],j:usize,inverse:bool,last:bool){
    ...
    let cc:Vec<_>=(0..=3).map(|i|if last{if i==1{one()}else{Vec::new()}}else if i<=1{Vec::new()}else{ceq_small(rank,c,i)}).collect();let aa0=aeq_small(rank,a,0);let aa1=aeq_small(rank,a,1);
    let spec3=mul(&aeq_small(rank,a,253),&cc[1]);let spec2=mul(&aeq_small(rank,a,254),&cc[1]);let spec1=mul(&aeq_small(rank,a,254),&cc[2]);
    ...
    if !last{for k in 0..3{let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v9::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs,g,scratch);}}
    let poly_start=circ.b.ops.len();
    ...
    for shift in 0..=2{
        ...
        let vv:[Poly<'_>;3]=std::array::from_fn(|k|{
            ...
            if index>=3&&index-3<=254{let cond=mul(&aeq_small(rank,a,index-3),&xor(one(),cc[1].clone()));v=replace(v,cond,false);}
            if index==257{v=replace(v,mul(&aeq_small(rank,a,254),&cc[1]),true);}v
        });
    ...
    if !last{for k in(0..3).rev(){let mut cs=vec![(g,true),(&sm[2],false)];if k==0{cs.push((&sm[1],false));}super::q793_t10_routes_v9::digit(circ,rank,a,c,w1,&q[k],dirty,1-k as isize,&cs,g,scratch);}}
    if std::env::var("Q793_T10_COST").ok().as_deref()==Some("1"){for(label,l,r)in[("cargo+qborrow",start,poly_start),("poly",poly_start,poly_end),("qreturn",poly_end,circ.b.ops.len())]{let ops=&circ.b.ops[l..r];let t=ops.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();eprintln!("Q793_T10_V9_CODEC last={last} inverse={inverse} part={label} ops={} T={t}",ops.len());}}
