...
pub(super) fn gather_a<'a>(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&'a[QReg],offset:usize,dirty:&[QReg])->(&'a QReg,Vec<crate::circuit::Op>){
    ...
    if let Some((lo,hi))=circ.q797_a_support {
        ...
        let mut nodes:Vec<_>=(0..256).map(|v|{if std::env::var("Q792_G_DBG").ok().as_deref()==Some("1")&&four&&v+offset>=255&&(lo..hi).contains(&v){eprintln!("Q792_G_DBG v={v} offset={offset} support={:?}",(lo,hi));}if ((lo..hi).contains(&v)||v==sentinel)&&!(super::q796_parity::enabled()&&offset==3&&v==sentinel)&&(!four||v+offset<255){Some(&word[v+offset])}else{None}}).collect();
        for level in 0..8 {
