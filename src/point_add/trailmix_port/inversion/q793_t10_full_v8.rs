...
use super::q793_t10_expand_v8::ceq;
...
fn branch(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],n:usize,j:usize,last:bool,low:bool){
    ...
    if low{super::q793_t10_expand_v8::flags(circ,rank,a,c,g,&sm[1],&sm[2],dirty);super::q793_t10_expand_v8::emit(circ,rank,a,c,sm,g,w1,w2,dirty,j,false,last);}
    ...
    if low{super::q793_t10_expand_v8::emit(circ,rank,a,c,sm,g,w1,w2,dirty,j,true,last);super::q793_t10_expand_v8::flags(circ,rank,a,c,g,&sm[1],&sm[2],dirty);}
    guard(circ,rank,a,c,sm,p1,p2,g,dirty,j,last,low);
    cost(circ,&mut cost_at,last,low,"encode+return");
}
fn cost(circ:&Circuit,at:&mut usize,last:bool,low:bool,part:&str){if std::env::var("Q793_T10_COST").ok().as_deref()==Some("1"){let ops=&circ.b.ops[*at..];let t=ops.iter().filter(|o|o.kind==crate::circuit::OperationType::CCX).count();eprintln!("Q793_T10_V8_COST last={last} low={low} part={part} ops={} T={t}",ops.len());}*at=circ.b.ops.len();}
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],c:&[QReg],sm:&[QReg],p1:&QReg,p2:&QReg,w1:&[QReg],w2:&[QReg],helpers:&[QReg],n:usize,j:usize){
    ...
    for op in &circ.b.ops[start..]{for h in [256,257,258]{let q=w1[h].id()as u64;assert!(op.q_target.0!=q&&op.q_control1.0!=q&&op.q_control2.0!=q,"T10 V8 touched omitted W1[{h}]");}}
