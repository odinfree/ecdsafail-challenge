...
pub fn run() {
    ...
    for block in 0..26 {
        ...
        circ.q797_a_support=Some(super::metadata_entry_head5::A_SUPPORTS[block]);
        super::q795_t10::emit(&mut circ,&rank,&a,&c,&p1,&p2,&w1,&w2,&helpers,n);
        ...
        for pattern in 0..4 {for batch in 0..rows.len().div_ceil(64) {
            ...
            for lane in 0..64 {
                ...
                let second=if cv==1{w1[width].id()as usize}else{w2[width+1].id()as usize};
                assert_eq!(before[second]>>lane&1!=0,cv==1,"second cargo promise");
                let cargo2=rnd(&mut seed)&1!=0;put(&mut before,second,lane,cargo2);put(&mut after,second,lane,cargo2);
                let mut carry=false;
        ...
        eprintln!("Q795_T10_DUAL_BLOCK block={block} ops={} T={} physical=565 PASS",b.ops.len(),b.ops.iter().filter(|o|o.kind==K::CCX).count());
    }
    eprintln!("Q795_T10_DUAL_PASS lanes={lanes} active={active}; two independent passenger cargos retained; not full step or physical Q795");
}
