...
pub(super) fn gather_a<'a>(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&'a[QReg],offset:usize,dirty:&[QReg])->(&'a QReg,Vec<crate::circuit::Op>){
    ...
    let sentinel=255usize.saturating_sub(usize::from(super::q793_lifecycle_r03::four_hole()));
    let four=super::q793_lifecycle_r03::four_hole();
    let mut nodes:Vec<_>=(0..256).map(|v|if v+offset<word.len()&&((lo..hi).contains(&v)||v==sentinel)&&!(offset==3&&v==sentinel)&&(!four||v+offset<255){Some(&word[v+offset])}else{None}).collect();
