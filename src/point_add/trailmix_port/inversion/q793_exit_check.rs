...
fn active() {
    ...
    crate::point_add::trailmix_port::inversion::q793_metadata_exit::exit_phase_cargo(&mut circ,&rank,&a,&c,&sm,&p1,&p2,&it,&w1,&w2,&dirty,0,256);
    ...
    eprintln!("Q793_DUAL_EXIT_BUILT ops={} T={}",builder.ops.len(),builder.ops.iter().filter(|o|o.kind==K::CCX).count());
    ...
    eprintln!("Q793_DUAL_EXIT_PASS lanes={total} scalar_rows={} first_exit_lanes={first} newA1_lanes={small} newA255_lanes={terminal}; new dual-cargo exit/length path; all4 passenger pairs plus random; three omitted lanes untouched, literal inverse and all dirty helpers restored; full step and whole Q not yet verified",rows.len());
}
...
fn offguard() {
    ...
    for op in &b.ops{op.validate();assert!(matches!(op.kind,K::X|K::CX|K::CCX));for i in [256,257,258]{let q=w1[i].id()as u64;assert!(op.q_target.0!=q&&op.q_control1.0!=q&&op.q_control2.0!=q);}}
    eprintln!("Q793_DUAL_EXIT_OFFGUARD_BUILT ops={} T={}",b.ops.len(),b.ops.iter().filter(|o|o.kind==K::CCX).count());
    ...
    for source in 0..3 {
        ...
        for pattern in 0..4 {for batch in 0..count.div_ceil(64) {
            ...
            for lane in 0..64 {
                ...
                set(&mut before,if av==255{&w2[258]}else{&w1[av+1]},lane,false);
            }
        ...
        eprintln!("Q793_DUAL_EXIT_OFFGUARD_SOURCE source={source} cases={count} PASS");
    }
    eprintln!("Q793_DUAL_EXIT_OFFGUARD_PASS lanes={total} scalar_offguard_rows={} birth_lanes={births}; arbitrary whole words and all dirty helpers, every rank/A/disabled-phase combination, all149 terminal histories, exact phase and inverse, omitted lanes untouched",rows.len());
}


pub fn run(){if std::env::var("LOWQ_Q793_NATIVE_MODE").ok().as_deref()==Some("dual-exit-offguard"){offguard();}else{active();}}
