...
pub(super) fn run(){
    ...
    for normalized in [false,true]{for j in 0..4{
        ...
        let nt=ops.iter().filter(|o|o.kind==K::CCX).count();eprintln!("Q793_R01_CFLAGS_CORE_BUILT j={j} ops={} T={nt}",ops.len());
        ...
        for pattern in 0..6{for batch in 0..rows.len().div_ceil(64){
            ...
            if sim.qubits!=after{let diffs:Vec<_>=sim.qubits.iter().zip(&after).enumerate().filter(|(_, (a,b))|a!=b).map(|(i,(a,b))|(i,format!("{:016x}",a^b))).collect();panic!("Q793 R01 normal v5 normalized={normalized} j={j} pattern={pattern} batch={batch} diffs={diffs:?}");}
    ...
    println!("{{\"kind\":\"Q793 R05 compact-C-flag normal R01 core\",\"active_lanes\":{total},\"offguard_lanes\":{off},\"shift0_1_2_ge3\":{counts:?},\"A2\":{a2},\"A252_253\":{high_a},\"results\":[{}],\"zero_lease_wrappers_tested\":false,\"whole_q793\":false}}",rows_out.join(","));
    eprintln!("Q793_R01_CFLAGS_CORE_PASS active={total} offguard={off}; native phase/inverse,three physical holes, low3 qpre/borrow, A2head/two-passenger/highA, external leases only; no wholeQ793 claim");
}
