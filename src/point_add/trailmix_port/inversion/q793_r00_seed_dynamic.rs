...
pub(super) fn emit(circ:&mut Circuit,rank:&[QReg],a:&[QReg],w1:&[QReg],w2:&[QReg],mask:&QReg,carry:&QReg,dirty:&[QReg],j:usize) {
    ...
    let mut anf:Vec<_>=(0..8192).map(|z|{
    ...
    for bit in 0..13{for m in 0..8192{if m>>bit&1!=0{anf[m]^=anf[m^(1<<bit)];}}}
    ...
    for(m,on)in anf.into_iter().enumerate(){
        ...
        for f in 0..4{if m>>(9+f)&1!=0{
            let av=[0usize,1,2,254][f];let rv=if f==3{29}else{0};
