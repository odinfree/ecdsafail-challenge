//! Own two-hole cargo selectors. Truncated physical words prune every missing leaf.
...
pub(super) fn across_a_terms(circ:&mut Circuit,rank:&[QReg],a:&[QReg],left_word:&[QReg],left_offset:usize,right_word:&[QReg],right_offset:usize,terms:&[Vec<(&QReg,bool)>],flip_left:bool,dirty:&[QReg]) {
    let(left,l)=super::q794_handoffs::gather_a(circ,rank,a,left_word,left_offset,dirty);
    let(right,r)=super::q794_handoffs::gather_a(circ,rank,a,right_word,right_offset,dirty);
    circ.cx(right,left);
...
pub(super) fn flip_a_terms(circ:&mut Circuit,rank:&[QReg],a:&[QReg],word:&[QReg],offset:usize,terms:&[Vec<(&QReg,bool)>],dirty:&[QReg]) {
    let(root,ops)=super::q794_handoffs::gather_a(circ,rank,a,word,offset,dirty);
