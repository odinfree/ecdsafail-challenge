...
fn compare_geq_p_secp256k1_inner(circ: &mut Circuit, a: &[QReg], flag: BorrowedQReg<'_>) {
    if std::env::var("Q795_COMPARE_PEBBLE").ok().as_deref()==Some("1"){
        compare_geq_p_pebbled(circ,a,flag);return;
    }
    assert!(a.len() == 257);
...
// Keep only the low10 predicate across the long high-prefix computation.
// Its six intermediate bits are cleaned, then recomputed only for cleanup.
struct LowPParts {low4:QReg,tail:QReg,low6:QReg,or67:QReg,or89:QReg,hi4:QReg}
fn low_p_parts(circ:&mut Circuit,a:&[QReg])->LowPParts{
    use crate::point_add::trailmix_port::arith::khattar_gidney::xor_and_of_khattar_gidney;
    let low4=circ.alloc_qreg("pebble.low4");xor_and_of_khattar_gidney(circ,&a[..4],&low4);
    let tail=circ.alloc_qreg("pebble.tail");circ.cx(&a[4],&tail);circ.cx(&low4,&tail);circ.ccx(&a[4],&low4,&tail);
    let low6=circ.alloc_qreg("pebble.low6");circ.ccx(&a[5],&tail,&low6);
    let or67=circ.alloc_qreg("pebble.or67");circ.cx(&a[6],&or67);circ.cx(&a[7],&or67);circ.ccx(&a[6],&a[7],&or67);
    let or89=circ.alloc_qreg("pebble.or89");circ.cx(&a[8],&or89);circ.cx(&a[9],&or89);circ.ccx(&a[8],&a[9],&or89);
    let hi4=circ.alloc_qreg("pebble.hi4");circ.cx(&or67,&hi4);circ.cx(&or89,&hi4);circ.ccx(&or67,&or89,&hi4);
    LowPParts{low4,tail,low6,or67,or89,hi4}
}
fn clear_low_p_parts(circ:&mut Circuit,a:&[QReg],s:LowPParts){
    use crate::point_add::trailmix_port::arith::khattar_gidney::xor_and_of_khattar_gidney;
    mbu_uncompute_or(circ,s.hi4,&s.or67,&s.or89);
    mbu_uncompute_or(circ,s.or89,&a[8],&a[9]);
    mbu_uncompute_or(circ,s.or67,&a[6],&a[7]);
    mbu_uncompute_and(circ,s.low6,&a[5],&s.tail);
    mbu_uncompute_or(circ,s.tail,&a[4],&s.low4);
    xor_and_of_khattar_gidney(circ,&a[..4],&s.low4);drop(s.low4);
}
fn compare_geq_p_pebbled(circ:&mut Circuit,a:&[QReg],flag:BorrowedQReg<'_>){
    use crate::point_add::trailmix_port::arith::khattar_gidney::xor_and_of_khattar_gidney;
    assert_eq!(a.len(),257);
    let s=low_p_parts(circ,a);
    let low10=circ.alloc_qreg("pebble.low10");circ.cx(&s.low6,&low10);circ.cx(&s.hi4,&low10);circ.ccx(&s.low6,&s.hi4,&low10);
    clear_low_p_parts(circ,a,s);
    let mid=circ.alloc_qreg("pebble.mid");xor_and_of_khattar_gidney(circ,&a[10..32],&mid);
    let midlow=circ.alloc_qreg("pebble.midlow");circ.ccx(&mid,&low10,&midlow);
    let tail=circ.alloc_qreg("pebble.tail32");circ.cx(&a[32],&tail);circ.cx(&midlow,&tail);circ.ccx(&a[32],&midlow,&tail);
    let high=circ.alloc_qreg("pebble.high");xor_and_of_khattar_gidney(circ,&a[33..256],&high);
    let joined=circ.alloc_qreg("pebble.joined");circ.ccx(&high,&tail,&joined);
    circ.cx(&a[256],&flag);circ.cx(&joined,&flag);circ.ccx(&a[256],&joined,&flag);
    if let BorrowedQReg::Owned(f)=flag{circ.zero_and_free(f);}
    mbu_uncompute_and(circ,joined,&high,&tail);
    xor_and_of_khattar_gidney(circ,&a[33..256],&high);drop(high);
    mbu_uncompute_or(circ,tail,&a[32],&midlow);
    mbu_uncompute_and(circ,midlow,&mid,&low10);
    xor_and_of_khattar_gidney(circ,&a[10..32],&mid);drop(mid);
    let s=low_p_parts(circ,a);mbu_uncompute_or(circ,low10,&s.low6,&s.hi4);clear_low_p_parts(circ,a,s);
}
...
fn compare_geq_half_p_secp256k1_inner(circ: &mut Circuit, a: &[QReg], flag: BorrowedQReg<'_>) {
    if std::env::var("Q795_COMPARE_PEBBLE").ok().as_deref()==Some("1"){
        compare_geq_half_p_pebbled(circ,a,flag);return;
    }
    assert!(a.len() == 256);
    use crate::point_add::trailmix_port::arith::khattar_gidney::xor_and_of_khattar_gidney;
...
struct LowHalfParts{or56:QReg,or78:QReg,hi4:QReg,low5:QReg}
fn low_half_parts(circ:&mut Circuit,a:&[QReg])->LowHalfParts{
    let or56=circ.alloc_qreg("pebble.half.or56");circ.cx(&a[5],&or56);circ.cx(&a[6],&or56);circ.ccx(&a[5],&a[6],&or56);
    let or78=circ.alloc_qreg("pebble.half.or78");circ.cx(&a[7],&or78);circ.cx(&a[8],&or78);circ.ccx(&a[7],&a[8],&or78);
    let hi4=circ.alloc_qreg("pebble.half.hi4");circ.cx(&or56,&hi4);circ.cx(&or78,&hi4);circ.ccx(&or56,&or78,&hi4);
    let low5=circ.alloc_qreg("pebble.half.low5");circ.ccx(&a[4],&a[3],&low5);
    LowHalfParts{or56,or78,hi4,low5}
}
fn clear_low_half_parts(circ:&mut Circuit,a:&[QReg],s:LowHalfParts){
    mbu_uncompute_and(circ,s.low5,&a[4],&a[3]);
    mbu_uncompute_or(circ,s.hi4,&s.or56,&s.or78);
    mbu_uncompute_or(circ,s.or78,&a[7],&a[8]);
    mbu_uncompute_or(circ,s.or56,&a[5],&a[6]);
}
fn compare_geq_half_p_pebbled(circ:&mut Circuit,a:&[QReg],flag:BorrowedQReg<'_>){
    use crate::point_add::trailmix_port::arith::khattar_gidney::xor_and_of_khattar_gidney;
    assert_eq!(a.len(),256);
    let s=low_half_parts(circ,a);
    let low9=circ.alloc_qreg("pebble.half.low9");circ.cx(&s.hi4,&low9);circ.cx(&s.low5,&low9);circ.ccx(&s.hi4,&s.low5,&low9);
    clear_low_half_parts(circ,a,s);
    let mid=circ.alloc_qreg("pebble.half.mid");xor_and_of_khattar_gidney(circ,&a[9..31],&mid);
    let branch=circ.alloc_qreg("pebble.half.branch");circ.ccx(&mid,&low9,&branch);
    let tail=circ.alloc_qreg("pebble.half.tail31");circ.cx(&a[31],&tail);circ.cx(&branch,&tail);circ.ccx(&a[31],&branch,&tail);
    let high=circ.alloc_qreg("pebble.half.high");xor_and_of_khattar_gidney(circ,&a[32..255],&high);
    let joined=circ.alloc_qreg("pebble.half.joined");circ.ccx(&high,&tail,&joined);
    circ.cx(&a[255],&flag);circ.cx(&joined,&flag);circ.ccx(&a[255],&joined,&flag);
    if let BorrowedQReg::Owned(f)=flag{circ.zero_and_free(f);}
    mbu_uncompute_and(circ,joined,&high,&tail);
    xor_and_of_khattar_gidney(circ,&a[32..255],&high);drop(high);
    mbu_uncompute_or(circ,tail,&a[31],&branch);
    mbu_uncompute_and(circ,branch,&mid,&low9);
    xor_and_of_khattar_gidney(circ,&a[9..31],&mid);drop(mid);
    let s=low_half_parts(circ,a);mbu_uncompute_or(circ,low9,&s.hi4,&s.low5);clear_low_half_parts(circ,a,s);
}
...
/// Same exact HMR phase correction with an independently supplied clean lane.
/// The lane is restored to zero and remains owned by its caller. Because it
/// is not deallocated here, a direct second comparison can uncompute it; the
/// allocated-carry retention workaround and third comparison are unnecessary.
pub fn compare_geq_p_secp256k1_phase_correction_mbu_with_lender(
    circ:&mut Circuit,a:&[QReg],q_to_hmr:QReg,clean_lender:&QReg,
) {
    assert_ne!(q_to_hmr.id(),clean_lender.id());
    assert!(a.iter().all(|q|q.id()!=clean_lender.id()));
    compare_geq_p_secp256k1(circ,a,clean_lender);
    circ.declare_identity(&q_to_hmr,clean_lender);
    let bit=circ.alloc_bit();circ.hmr(&q_to_hmr,bit);circ.z_if_bit(clean_lender,bit);circ.free_bit(bit);
    drop(q_to_hmr);
    compare_geq_p_secp256k1(circ,a,clean_lender);
}
