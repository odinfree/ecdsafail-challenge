...
pub fn unary_iterate_log_star<F>(circ: &mut Circuit, c: &[&QReg], n_iters: usize, body: F)
where
    F: FnMut(&mut Circuit, usize, &QReg),
{
...
pub fn unary_iterate_log_star_with_clean_lender<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
...
pub fn unary_iterate_log_star_toggle_target_with_clean_lender<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    clean_lender: Option<&QReg>,
...
pub fn unary_iterate_log_star_toggle_target_with_clean_lenders<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    clean_lenders: &[&QReg],
...
pub fn unary_iterate_direct_toggle_target_with_clean_scratch<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    clean_scratch: &[&QReg],
...
pub fn unary_iterate_direct_toggle_target_with_dirty_scratch<F>(
    circ: &mut Circuit,
    c: &[&QReg],
    n_iters: usize,
    dirty: &[&QReg],
...
struct Sub800UlsProofEnvironment {
    dirty_mcx3: Option<std::ffi::OsString>,
}
...
fn build_sub800_uls_proof_harness(
    ...
    let external_ids: Vec<u32> = counter
        .iter()
        .chain(result.iter())
...
struct Sub800UlsDirectSelectorProofHarness {
    ...
    result_ids: Vec<u32>,
    lender_id: u32,
    cursor_scratch_ids: Vec<u32>,
    callback_order: Vec<Option<usize>>,
...
fn verify_sub800_uls_dirty_selector_primitive() -> (usize, usize) {
    ...
    let control_ids = controls.iter().map(QReg::id).collect::<Vec<_>>();
    let target_id = target.id();
    let dirty_ids = dirty.iter().map(QReg::id).collect::<Vec<_>>();
    let builder = circ.into_builder();
