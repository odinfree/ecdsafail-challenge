...
fn main() {
    ...
    if n_ops != VALIDATED_OPS {
        eprintln!(
            "pingpong_filter: WARNING -- this circuit emits {n_ops} ops, but the classical model \
             was validated against {VALIDATED_OPS}.  The tuned windows in `Model::new` mirror \
             `pingpong_div.rs` defaults; if they moved, this filter will report FALSE NEGATIVES \
             (islands silently discarded).  Re-validate against eval_circuit before trusting it."
        );
    }
