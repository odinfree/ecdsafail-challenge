...
//! "classical mismatches" count *exactly*, without simulating the 13.4M-op
...
//!
//! Imported from odinfree/pingpong-prefilter commit
//! `da0e5721f14f5d956aa703b38311f4087388a7b1`.  This checkout has since
//! changed its width schedule, fused early rounds, and replay frame.  Until a
//! native model is revalidated, this binary deliberately accepts only the
//! explicit compatibility profile checked by `require_compatibility_profile`.
...
const PIJ: [usize; 24] = [10, 7, 11, 17, 18, 3, 5, 16, 8, 21, 24, 4, 15, 23, 19, 13, 12, 2, 20, 14, 22, 9, 6, 1];
...
impl Sponge {
    fn new() -> Self {
        Sponge { st: [0u64; 25], buf: [0u8; RATE], len: 0 }
    }
    #[inline]
...
const PMOD: U4 = [0xfffffffefffffc2f, 0xffffffffffffffff, 0xffffffffffffffff, 0xffffffffffffffff];
const ZERO4: U4 = [0, 0, 0, 0];
const ONE4: U4 = [1, 0, 0, 0];
const GX: U4 = [0x59f2815b16f81798, 0x029bfcdb2dce28d9, 0x55a06295ce870b07, 0x79be667ef9dcbbac];
const GY: U4 = [0x9c47d08ffb10d4b8, 0xfd17b448a6855419, 0x5da4fbfc0e1108a8, 0x483ada7726a3c465];
...
const JINF: Jac = Jac { x: ONE4, y: ONE4, z: ZERO4 };
...
impl CombTable {
    fn build() -> Self {
        ...
        let mut base = (GX, GY);
        for _w in 0..32 {
            let mut acc = Jac { x: base.0, y: base.1, z: ONE4 };
            jacs.push(acc);
            for _d in 2..=255 {
                acc = jac_add_affine(&acc, &base.0, &base.1);
                jacs.push(acc);
            }
            let mut t = Jac { x: base.0, y: base.1, z: ONE4 };
            for _ in 0..8 {
                t = jac_double(&t);
            }
        ...
        CombTable { pts: batch_to_affine(&jacs) }
    }
...
fn w_andnot(a: &W5, m: &W5) -> W5 {
    [a[0] & !m[0], a[1] & !m[1], a[2] & !m[2], a[3] & !m[3], a[4] & !m[4]]
}
#[inline(always)]
...
#[inline(always)]
fn u4_shl1(a: &U4) -> U4 {
    [a[0] << 1, (a[1] << 1) | (a[0] >> 63), (a[2] << 1) | (a[1] >> 63), (a[3] << 1) | (a[2] >> 63)]
}

/// Tuning knobs of the emitted circuit that have a CLASSICAL effect.
/// Read from the same env vars `pingpong_div.rs` reads, so a build override
/// moves the circuit and the model together.
struct Model {
    rounds_div: usize,
    rounds_mul: usize,
...
fn require_compatibility_profile() {
    let required = [
        ("SUB4_PP_ROUNDS", "696"),
        ("SUB4_PP_ROUNDS_MUL", "696"),
        ("SUB4_PP_SCHED_LINEAR", "1"),
        ("SUB4_PP_WIDTH_RESCALE", "1"),
        ("SUB4_PP_SIGNED_FRAME", "0"),
        ("SUB4_PP_FUSE_ROUND1", "0"),
        ("SUB4_PP_ROUND0_SPARSE_FWD", "0"),
    ];
    let wrong: Vec<String> = required
        .iter()
        .filter_map(|&(name, want)| {
            let got = std::env::var(name).ok();
            (got.as_deref() != Some(want)).then(|| {
                format!("{name}={} (required {want})", got.as_deref().unwrap_or("<unset>"))
            })
        })
        .collect();
    if !wrong.is_empty() {
        eprintln!(
            "pingpong_filter: refusing an unvalidated source profile:\n  {}",
            wrong.join("\n  ")
        );
        std::process::exit(2);
    }
...
impl Model {
    fn new() -> Self {
        require_compatibility_profile();
        if std::env::var_os("SUB4_PINGPONG_SEPARATE_LIFT").is_some() {
            eprintln!("pingpong_filter: SUB4_PINGPONG_SEPARATE_LIFT is not modelled; refusing to guess.");
            std::process::exit(2);
        }
        ...
        // of them mirrors a `tuned_window(...)` / `rounds_for(...)` default in
        // `src/point_add/pingpong_div.rs`; the circuit is the authority and a
        // stale copy here is a silent FALSE-NEGATIVE source (the filter calls a
        // shot clean that `eval_circuit` calls dirty, so a valid island is
        // discarded).  Re-check all four against `pingpong_div.rs` after ANY
        // change to that file.  Measured drift that this comment exists to
        // prevent: at nonce 10 the previous copy (699 / 55 / 20) reported 10
        // classical mismatches where eval_circuit reports 13.
        //   pingpong_div.rs `rounds()`                 -> 700
        //   pingpong_div.rs `rounds_for(Multiply)`     -> rounds() - 2
        ...
        let chunk_w = env_usize("SUB4_PP_REPLAY_CHUNK_COMPARE", 21);
        assert!(fold_w >= 36 && fold_w <= 64, "REPLAY_FOLD_WINDOW {fold_w} outside the modelled range");
        ...
        let rescale = std::env::var_os("SUB4_PP_WIDTH_RESCALE").is_some();
        ...
        let wid_div = (0..rounds_div).map(|r| value_width(width_round_index(r, rounds_div))).collect();
        // The multiply traversal shares `rounds()` for the width schedule
        // (width_round_index uses rounds(), i.e. the DIVIDE round count).
        let wid_mul = (0..rounds_mul).map(|r| value_width(width_round_index(r, rounds_div))).collect();
        let masks = (0..=VALUE_WIDTH).map(mask_w).collect();
        Model { rounds_div, rounds_mul, fold_w: fold_w as u32, flag_w, chunk_w, endpoint_m, seed_m, wid_div, wid_mul, masks }
    }
}
...
fn sigma_add_n<const NL: usize>(
    ...
    let m = &masks[w];
    let tp = if sign == 1 { w_xor_n::<NL>(tgt, m) } else { *tgt };
...
fn fused_lift_round0_reverse(v: &mut W5, a0: u64, fold_w: u32) {
    ...
    *v = w_and(&sh, &mask_w(VALUE_WIDTH));
    ...
    let fm: u64 = if fold_w >= 64 { u64::MAX } else { (1u64 << fold_w) - 1 };
...
#[inline]
fn walk_back_round(u: &mut W5, v: &mut W5, w_prev: usize, r: usize, sign: u64, w: usize, m: &Model) {
    match (w + 63) / 64 {
...
fn const_trunc(acc: &U4, c: u64, mbits: usize, sub: bool) -> U4 {
    ...
    let low: U4 = [lo[0] & mask[0], lo[1] & mask[1], lo[2] & mask[2], lo[3] & mask[3]];
    let cv: U4 = [c, 0, 0, 0];
    let r = if sub { u4_subb(&low, &cv).0 } else { u4_addc(&low, &cv).0 };
    for i in 0..4 {
...
fn flag_delta(sv: &U4, dv: &U4, w: usize) -> u64 {
    if w == 0 || w >= 256 { return 0; }
    let lo_bits = 256 - w;
    // top-w equality
    let (lw, lb) = (lo_bits / 64, lo_bits % 64);
    let mut top_eq = true;
    for i in lw..4 {
        let sm = if i == lw && lb != 0 { sv[i] >> lb } else { sv[i] };
        let dm = if i == lw && lb != 0 { dv[i] >> lb } else { dv[i] };
        if sm != dm { top_eq = false; break; }
    }
    if !top_eq { return 0; }
    // low (256-w) compare, s < d
    let mut sl = *sv; let mut dl = *dv;
    for i in lw..4 {
        if i == lw && lb != 0 { sl[i] &= (1u64 << lb) - 1; dl[i] &= (1u64 << lb) - 1; }
        else if i > lw || lb == 0 { sl[i] = 0; dl[i] = 0; }
    }
    for i in (0..4).rev() {
        if sl[i] != dl[i] { return (sl[i] < dl[i]) as u64; }
    }
    0
}
...
fn smaph(sign: u64, src: &U4, tgt: &U4, fold_w: u32, flag_w: usize, nphase: &mut u64) -> U4 {
    ...
    *nphase += flag_delta(&acc, src, flag_w);
    ...
    fold_apply(&mut acc, fold_operand(minus_f, plus_2f, plus_f, fold_w), fold_w);
...
fn smdapf(sign: u64, src: &U4, tgt: &U4, fold_w: u32, flag_w: usize, nphase: &mut u64) -> U4 {
    ...
    *nphase += flag_delta(&acc2, src, flag_w);
    ...
    fold_apply(&mut acc2, fold_operand(minus_f, plus_2f, plus_f, fold_w), fold_w);
    if sign == 1 {
        u4_not(&acc2)
    } else {
        acc2
...
fn pingpong_divide(den: &U4, num: &U4, m: &Model, s: &mut Scratch, nphase: &mut u64) -> (U4, U4) {
    ...
    for r in 0..rounds {
        ...
        if let Some(rec) = s.rec.as_mut() {
            rec.push(("dwalk", r, [u[0], u[1], u[2], u[3]], [v[0], v[1], v[2], v[3]]));
        }
    }
    ...
    for r in (0..rounds).rev() {
        ...
        wp = wr;
        if let Some(rec) = s.rec.as_mut() {
            rec.push(("dback", r, [u[0], u[1], u[2], u[3]], [v[0], v[1], v[2], v[3]]));
        }
    }
    if wp < VALUE_WIDTH {
...
fn pingpong_multiply(den: &U4, num: &U4, m: &Model, s: &mut Scratch, nphase: &mut u64) -> (U4, U4) {
    ...
    for r in 0..rounds {
        ...
        s.tape[r] = walk_round(&mut u, &mut v, r, w, m) as u8;
        if let Some(rec) = s.rec.as_mut() {
            rec.push(("mwalk", r, [u[0], u[1], u[2], u[3]], [v[0], v[1], v[2], v[3]]));
        }
    }
    ...
    for r in (0..rounds).rev() {
        ...
        wp = wr;
        if let Some(rec) = s.rec.as_mut() {
            rec.push(("mback", r, [u[0], u[1], u[2], u[3]], [v[0], v[1], v[2], v[3]]));
        }
    }
    if wp < VALUE_WIDTH {
        sign_extend(&mut v, wp, VALUE_WIDTH, &m.masks);
...
fn point_add_classical_traced(
    tx: &U4, ty: &U4, ox: &U4, oy: &U4, m: &Model, s: &mut Scratch,
...
/// Op count of the circuit the model in `Model::new` was validated against
/// (north / commit cc82b18, repo-default knobs).  The model hardcodes the
/// circuit's tuned windows, so a circuit whose op stream differs may well need
/// different windows -- and a stale window is a silent FALSE NEGATIVE (a valid
/// island discarded).  Warn loudly rather than screen a circuit we were never
/// checked against.  Re-validate against `eval_circuit` and update this
/// constant when the circuit legitimately moves.
/// Source fingerprint of the compatibility profile on research commit
/// `fc57c30`.  This is not a validation claim: evaluator cross-checks at
/// nonces 13 and 14 each exposed one additional classical failure outside the
/// high-level ping-pong model.
const SOURCE_BOUND_OPS: usize = 12_886_147;
...
impl Checkpoint {
    ...
    /// nonce WITHOUT the 13.4M-op stream.  Layout (all little-endian):
    ...
    fn build(ops: &[Op]) -> Self {
        ...
        Checkpoint { sponge: sp, tail_template, n_ops: ops.len() }
    }
...
fn screen_nonce(
    ...
    // 5. The screen.
    let dump_shot: Option<usize> = std::env::var("PF_DUMP_ROUNDS").ok().and_then(|v| v.parse().ok());
    ...
    for i in 0..n {
        ...
        if Some(i) == dump_shot {
            ...
            let mut nph0 = 0u64;
            let _ = point_add_classical(&s.tx[i], &s.ty[i], &s.ox[i], &s.oy[i], m, &mut s.model, &mut nph0);
        ...
        if trace.contains(&i) {
            ...
            {
                eprintln!("TRACE {nonce} {i} {name} {} {}", hexbe_trim(&a), hexbe_trim(&b));
            }
            eprintln!(
                "TRACE {nonce} {i} INPUT tx={} ty={} ox={} oy={}",
                hexbe_trim(&s.tx[i]), hexbe_trim(&s.ty[i]), hexbe_trim(&s.ox[i]), hexbe_trim(&s.oy[i])
            );
        }
        let mut nph = 0u64;
        let (gx, gy) = point_add_classical(&s.tx[i], &s.ty[i], &s.ox[i], &s.oy[i], m, &mut s.model, &mut nph);
        ...
        if gx != ex || gy != ey {
            ...
            if verbose {
                eprintln!(
                    ...
                    hexbe_trim(&gx), hexbe_trim(&gy), hexbe_trim(&ex), hexbe_trim(&ey)
                );
            }
        }
    }
    if timing {
        eprintln!(
            ...
            t_xof, t_ec, tt.elapsed().as_secs_f64()
        );
    }
...
fn main() {
    ...
    let mut jobs: usize = std::thread::available_parallelism().map(|v| v.get()).unwrap_or(4);
    ...
    if n_ops != SOURCE_BOUND_OPS {
        eprintln!(
            "pingpong_filter: refusing circuit with {n_ops} ops; compatibility profile binds \
             {SOURCE_BOUND_OPS}. Regenerate and revalidate the model before screening."
        );
        std::process::exit(2);
    }
    if std::env::var("PINGPONG_FILTER_DIAGNOSTIC_ONLY").ok().as_deref() != Some("1") {
        eprintln!(
            "pingpong_filter: current-source evaluator gate is not closed. Set \
             PINGPONG_FILTER_DIAGNOSTIC_ONLY=1 only for bounded model diagnostics; do not \
             use this build to reject nonces."
        );
        std::process::exit(2);
    }
...
fn run_selftest(cp: &Checkpoint, comb: &CombTable, m: &Model) {
    ...
    eprintln!(
        ...
        cp.sponge.len,
        cp.sponge.buf[..16.min(cp.sponge.len)].iter().map(|b| format!("{b:02x}")).collect::<String>()
    );
    ...
    let lam = fe_mul(&fe_sub(&a[1].1, &a[0].1), &fe_inv(&fe_sub(&a[1].0, &a[0].0)));
    ...
    let mut sc = Scratch { tape: vec![0u8; m.rounds_div.max(m.rounds_mul) + 8], rec: None };
    let mut nph_st = 0u64;
    let (gx, gy) = point_add_classical(&a[0].0, &a[0].1, &a[1].0, &a[1].1, m, &mut sc, &mut nph_st);
    eprintln!("gx = {}  {}", hexbe(&gx), if gx == ex { "OK" } else { "MISMATCH" });
    eprintln!("gy = {}  {}", hexbe(&gy), if gy == ey { "OK" } else { "MISMATCH" });
}
