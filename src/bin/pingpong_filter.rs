...
impl Model {
    fn new() -> Self {
        ...
        //   pingpong_div.rs `rounds_for(Multiply)`     -> rounds() - 4
        //   pingpong_div.rs `replay_fold_window()`     -> 53
        //   pingpong_div.rs `endpoint_fold_window()`   -> 20
        let rounds_div = env_usize("SUB4_PP_ROUNDS", 700);
        let rounds_mul = env_usize("SUB4_PP_ROUNDS_MUL", rounds_div - 4);
        let fold_w = env_usize("SUB4_PP_REPLAY_FOLD_WINDOW", 53);
        ...
        let flag_w = env_usize("SUB4_PP_REPLAY_FLAG_COMPARE", 20);
...
const VALIDATED_OPS: usize = 12_822_408;
...
fn screen_nonce(
    ...
    let mut fault_mask = if faultshots { vec![0u64; FAULT_MASK_WORDS] } else { Vec::new() };
...
fn main() {
    ...
    if n_ops != VALIDATED_OPS {
        eprintln!(
            "pingpong_filter: REFUSING TO RUN -- this circuit emits {n_ops} ops, but the \
             source-bound B1=24 model requires {VALIDATED_OPS}. Rebuild and re-qualify the model."
        );
        std::process::exit(4);
    }
    ...
    if let Some(path) = nonce_file.as_ref() {
        ...
        let out: Mutex<Vec<(u64, usize, usize, Vec<u64>)>> = Mutex::new(Vec::with_capacity(list.len()));
        std::thread::scope(|sc| {
            for _ in 0..jobs {
                sc.spawn(|| {
                    ...
                    loop {
                        ...
                        if i >= list.len() { break; }
                        let n = list[i];
                        let (cl, ph, mask) = screen_nonce(n, &cp, &comb, &m, &mut st, verbose, faultshots);
