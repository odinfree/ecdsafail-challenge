...
// d919/da61's default-on fused walk round 1. The low correction subtracts
// h=2^31+488 only inside the 52-bit borrow window, while the independent high
// increment adds 2^255 only inside the surviving signed-width slice.
PP_HD u64 pp_fused_round1_model(PP_S320* u, const PP_S320* v, int width) {
    ...
    if (sign) {
        int full = width / 64;
        int rem = width % 64;
        for (int i = 0; i < full; i++) q.v[i] = ~q.v[i];
        if (rem != 0) q.v[full] ^= (1ULL << rem) - 1;
    }

    const u64 low_mask = (1ULL << 52) - 1;
    const u64 h = (1ULL << 31) + 488;
    q.v[0] = (q.v[0] & ~low_mask) | ((q.v[0] - h) & low_mask);

    u64 old = q.v[3];
    q.v[3] += 1ULL << 63;
    bool carry = q.v[3] < old;
    for (int i = 4; carry && i * 64 < width; i++) {
        old = q.v[i];
        q.v[i] += 1;
        carry = q.v[i] < old;
    }
    pp_s320_truncate_signed(&q, width);
    *u = q;
    return sign;
}
...
PP_HD void pp_walk_sig(const u64 a[4], int rounds, bool* fault_out, bool* term_ok_out,
    ...
    for (int r = 0; r < rounds; r++) {
        ...
        if (r == 1) {
            u64 sign = pp_fused_round1_model(&u, &v, w);
...
PP_HD void pp_walk_sig_wrapped(const u64 a[4], int rounds, bool* u_neg_out,
...
// d919 signed replay frame.  The circuit enters this frame immediately before
// divide replay round 2 and leaves it after the final divide replay round.
// Canonical residues with bit 255 set are represented as value-p by adding f
// only through the source's 54-bit truncated carry window.
PP_HD void pp_to_signed_frame(u64 t[4]) {
    if ((t[3] >> 63) != 0) pp_trunc_add(t, PP_FC, 54);
}

PP_HD void pp_from_signed_frame(u64 t[4]) {
    if ((t[3] >> 63) != 0) pp_trunc_sub(t, PP_FC, 54);
}

// signed_mod_add_pm_halve_fused_signed at value level.  The 257th wire `hi`
// starts as the exact sign bit of V = target +/- source.  The low pseudo-
// Mersenne correction is deliberately truncated to 54 bits, matching
// fused_fold_maskfree; parity toggles hi for the +/-2^256 part of +/-p.
PP_HD void pp_halve_fused_signed(bool sign, const u64 s[4], u64 t[4]) {
    bool tneg = (t[3] >> 63) != 0;
    bool sneg = (s[3] >> 63) != 0;
    ...
    bool carry;
    if (!sign) {
        carry = pp_add256(t, s, low);
    } else {
        u64 ns[4];
        for (int i = 0; i < 4; i++) ns[i] = ~s[i];
        u64 one[4] = {1, 0, 0, 0};
        u64 twos[4];
        pp_add256(ns, one, twos);
        carry = pp_add256(t, twos, low);
    }

    // Circuit identity for bit 256 of the exact signed add/sub.  For
    // subtraction the complemented-adder carry has the opposite polarity.
    bool hi = tneg ^ sneg ^ carry;
    if (sign) hi = !hi;

    for (int i = 0; i < 4; i++) t[i] = low[i];
    bool parity = (t[0] & 1) != 0;
    if (parity) {
        if (hi) pp_fold54(t, -1); else pp_fold54(t, 1);
        hi = !hi;
    }
    pp_shr1_256(t);
    if (hi) t[3] |= 1ULL << 63;
...
PP_HD bool pp_wrapped_output_correct(const u64 tx[4], const u64 ty[4],
    ...
    bool du_neg, dv_neg;
    pp_walk_sig_wrapped(x2, PP_ROUNDS_DIV, &du_neg, &dv_neg, signs_d, wtab);
    u64 xd[4], y2d[4];
    ...
    bool mu_neg, mv_neg;
    pp_walk_sig_wrapped(x2c, PP_ROUNDS_MUL, &mu_neg, &mv_neg, signs_m, wtab);
    u64 xm[4], y2m[4];
