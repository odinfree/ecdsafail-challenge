...
static const u64 PP_EXPECTED_OPS_Q1272 = 12812623ULL; // exact da61d487 early-sign stream
static const u64 PP_EXPECTED_STATE_DIGEST_Q1272 = 0x3a16b78dd97e0960ULL;
...
inline void pp_load_prefix(const char* path, PP_Prefix* out) {
    ...
    if (n != PP_EXPECTED_OPS_Q1272) {
        fprintf(stderr,
                "ppgpu: FATAL: ops stream op count %llu != expected %llu (d919-early-sign); "
    ...
    if (digest != PP_EXPECTED_STATE_DIGEST_Q1272) {
        fprintf(stderr,
                ...
                "(d919-early-sign); refusing same-count unknown stream\n",
