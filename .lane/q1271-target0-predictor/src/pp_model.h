...
// retargeted to the exact Q1271 target0/sign-alias stream at a220903.  Both
...
#define PP_ROUNDS_DIV 696 // exact Q1271 alias divide depth @a220903
#define PP_ROUNDS_MUL 696 // exact Q1271 alias multiply depth @a220903
#define PP_REPLAY_PEAK_DIV 1271
#define PP_REPLAY_PEAK_MUL 1272
#define PP_SQUARE_LADDER 241
...
// The exact Q1271 source keeps the old repair table in Rust but guards it with
...
// Q1271 alias value_width(): round 0 stays VALUE_WIDTH. Other rounds
...
//   family 0: pingpong_div.rs:1577 (chunk-boundary carry erase)
//   family 1: pingpong_div.rs:1846 (divide replay flag erase)
//   family 2: pingpong_div.rs:2006 (multiply replay flag erase)
//
...
PP_HD bool pp_chunk_layout(int n, int target, bool final_carry, PP_ChunkLayout* out) {
    const int window = 20; // replay_chunk_compare() on frozen a220903
...
PP_HD int pp_replay_ladder_budget(bool multiply, int round) {
    ...
    int replay_peak = multiply ? PP_REPLAY_PEAK_MUL : PP_REPLAY_PEAK_DIV;
    int allowance = replay_peak - (tape_len + 2 * PP_N + 2 * walk_width);
...
PP_HD bool pp_halve_fused_phase(bool sign, const u64 s[4], u64 t[4], int round,
    ...
    // pingpong_div.rs:1846: Hmr(overflow) followed by the conditioned
...
PP_HD bool pp_double_add_fused_phase(bool sign, const u64 s[4], u64 t[4], int round,
    ...
    // pingpong_div.rs:2006, same post-fold / pre-uncomplement frame.
    bool repaired = pp_top_window_less4(t, s, 256, 22);
...
// pingpong_div.rs:1577 boundary sequence when the source selected the chunked
