...
//
// A function-by-function transliteration of the VALIDATED Q1272 CPU reference
//   src/bin/pingpong_filter.rs
// against structural commit 14608572.  Every device function below names the
// Rust function it mirrors; when in doubt, the Rust file is the authority.
//
...
#define EXPECTED_OPS_Q1272_SELECTOR 12908488ULL
...
// Circuit-exact coordinate shell and product-register square.  These mirror
// the generic value-channel corrections in the qualified Rust predictor.
#define COORD_FOLD_BITS 53
#define COORD_FOLD_MASK ((1ULL << COORD_FOLD_BITS) - 1ULL)
#define SQUARE_F_LSBS 56
#define SQUARE_GUARD 24

typedef u64 SquareWide[6];

__device__ __forceinline__ void coord_fold_f_complemented(u64 value[4]) {
    u64 low = value[0] & COORD_FOLD_MASK;
    u64 folded = ((low ^ COORD_FOLD_MASK) + FCV) ^ COORD_FOLD_MASK;
    value[0] = (value[0] & ~COORD_FOLD_MASK) | (folded & COORD_FOLD_MASK);
}

__device__ __forceinline__ void coord_sub_circuit(
    u64 out[4], const u64 reg[4], const u64 coord[4]) {
    u64 not_reg[4], sum[4];
    u4_not(not_reg, reg);
    u64 carry = u4_addc(sum, not_reg, coord);
    u4_not(out, sum);
    if (carry == 1) coord_fold_f_complemented(out);
}

__device__ __forceinline__ void coord_rsub_circuit(
    u64 out[4], const u64 reg[4], const u64 coord[4]) {
    const u64 one[4] = {1, 0, 0, 0};
    u64 coord_plus_one[4], not_reg[4];
    u4_addc(coord_plus_one, coord, one);
    u4_not(not_reg, reg);
    u64 carry = u4_addc(out, not_reg, coord_plus_one);
    if (carry == 0) coord_fold_f_complemented(out);
}

__device__ __forceinline__ void square_wide_zero(SquareWide out) {
#pragma unroll
    for (int i = 0; i < 6; ++i) out[i] = 0;
}

__device__ __forceinline__ void square_wide_from4(SquareWide out, const u64 a[4]) {
    out[0] = a[0]; out[1] = a[1]; out[2] = a[2]; out[3] = a[3];
    out[4] = 0; out[5] = 0;
}

__device__ __forceinline__ void square_wide_low4(u64 out[4], const SquareWide a) {
    out[0] = a[0]; out[1] = a[1]; out[2] = a[2]; out[3] = a[3];
}

__device__ __forceinline__ void square_wide_get(
    SquareWide out, const SquareWide v, int lo, int width) {
    square_wide_zero(out);
    if (width == 0) return;
    int limb = lo / 64;
    int offset = lo % 64;
    int nlimbs = (width + 63) / 64;
    for (int i = 0; i < nlimbs; ++i) {
        u64 value = (limb + i < 6) ? v[limb + i] : 0;
        if (offset != 0) {
            value = (value >> offset)
                | ((limb + i + 1 < 6) ? (v[limb + i + 1] << (64 - offset)) : 0);
        }
        out[i] = value;
    }
    if ((width % 64) != 0) out[width / 64] &= (1ULL << (width % 64)) - 1ULL;
    for (int i = width / 64 + 1; i < 6; ++i) out[i] = 0;
}

__device__ __forceinline__ void square_wide_set(
    SquareWide v, int lo, int width, const SquareWide value) {
    if (width == 0) return;
    int limb = lo / 64;
    int offset = lo % 64;
    int nlimbs = (width + 63) / 64;
    for (int i = 0; i < nlimbs; ++i) {
        ...
        if ((width % 64) != 0 && i == width / 64)
            chunk &= (1ULL << (width % 64)) - 1ULL;
        if (offset == 0) {
            u64 mask = ((width % 64) != 0 && i == width / 64)
                ? ((1ULL << (width % 64)) - 1ULL) : ~0ULL;
            if (limb + i < 6) v[limb + i] = (v[limb + i] & ~mask) | (chunk & mask);
        } else {
            u64 low = chunk << offset;
            u64 high = chunk >> (64 - offset);
            u64 low_mask = ((width % 64) != 0 && i == width / 64)
                ? (((1ULL << (width % 64)) - 1ULL) << offset)
                : (~0ULL << offset);
            if (limb + i < 6)
                v[limb + i] = (v[limb + i] & ~low_mask) | (low & low_mask);
            if (limb + i + 1 < 6) {
                u64 high_mask = ((width % 64) != 0 && i == width / 64)
                    ? (((1ULL << (width % 64)) - 1ULL) >> (64 - offset))
                    : (~0ULL >> (64 - offset));
                v[limb + i + 1] =
...
__device__ __forceinline__ u64 square_wide_add(
    SquareWide sum, const SquareWide a, const SquareWide b) {
    u64 carry = 0;
#pragma unroll
    for (int i = 0; i < 6; ++i) {
        u64 value = a[i] + b[i];
        u64 carry1 = value < a[i];
        u64 value2 = value + carry;
        u64 carry2 = value2 < value;
        sum[i] = value2;
        carry = carry1 | carry2;
    }
    return carry;
...
__device__ __forceinline__ void square_wide_mul(
    SquareWide product, const SquareWide a, const SquareWide b) {
    square_wide_zero(product);
    for (int i = 0; i < 6; ++i) {
        u64 carry = 0;
        for (int j = 0; j < 6 - i; ++j) {
            u64 lo = a[i] * b[j];
            u64 hi = mulhi64(a[i], b[j]);
            u64 value = product[i + j] + lo;
            hi += value < product[i + j];
            u64 value2 = value + carry;
            hi += value2 < value;
            product[i + j] = value2;
            carry = hi;
...
__device__ __forceinline__ void square_wide_mask(SquareWide mask, int width) {
#pragma unroll
    for (int i = 0; i < 6; ++i) {
        int remaining = width - 64 * i;
        mask[i] = remaining >= 64 ? ~0ULL
            : (remaining <= 0 ? 0 : ((1ULL << remaining) - 1ULL));
    }
}

__device__ __forceinline__ void square_wide_xor(
    SquareWide out, const SquareWide a, const SquareWide b) {
#pragma unroll
    for (int i = 0; i < 6; ++i) out[i] = a[i] ^ b[i];
}

__device__ __forceinline__ bool square_wide_bit(const SquareWide value, int bit) {
    return ((value[bit / 64] >> (bit % 64)) & 1ULL) == 1ULL;
}

__device__ __forceinline__ void square_f_window(SquareWide reg, bool control) {
    if (!control) return;
    SquareWide low, addend, sum;
    const u64 f[4] = {FCV, 0, 0, 0};
    square_wide_get(low, reg, 0, SQUARE_F_LSBS);
    square_wide_from4(addend, f);
    square_wide_add(sum, low, addend);
    square_wide_set(reg, 0, SQUARE_F_LSBS, sum);
}

__device__ __forceinline__ void square_mod_add_top(
    SquareWide out, const SquareWide value, int shift, bool subtract) {
    SquareWide mask256, accumulator, sum;
    square_wide_mask(mask256, 256);
    if (subtract) square_wide_xor(out, out, mask256);
    square_wide_get(accumulator, out, shift, 256 - shift);
    square_wide_add(sum, accumulator, value);
    bool overflow = square_wide_bit(sum, 256 - shift);
    square_wide_set(out, shift, 256 - shift, sum);
    square_f_window(out, overflow);
    if (subtract) square_wide_xor(out, out, mask256);
}

__device__ __forceinline__ void square_window_add(
    SquareWide out, const SquareWide value, int width, int shift, bool subtract) {
    int guarded_width = width + SQUARE_GUARD;
    SquareWide accumulator, mask, sum;
    square_wide_get(accumulator, out, shift, guarded_width);
    square_wide_mask(mask, guarded_width);
    if (subtract) square_wide_xor(accumulator, accumulator, mask);
    square_wide_add(sum, accumulator, value);
    if (subtract) square_wide_xor(accumulator, sum, mask);
    else {
#pragma unroll
        for (int i = 0; i < 6; ++i) accumulator[i] = sum[i];
    }
    square_wide_set(out, shift, guarded_width, accumulator);
}

__device__ __forceinline__ void square_apply_f(
    SquareWide out, const SquareWide value, int value_width, bool subtract) {
    const int shifts[5] = {0, 4, 6, 10, 32};
    const bool negates[5] = {false, false, true, false, false};
    bool sign = subtract;
#pragma unroll
    for (int i = 0; i < 5; ++i) {
        if (negates[i]) sign = !sign;
        square_window_add(out, value, value_width, shifts[i], sign);
        if (negates[i]) sign = !sign;
    }
}

__device__ __forceinline__ void square_apply_shift_half(
    SquareWide out, const SquareWide product, int product_width, bool subtract) {
    SquareWide low;
    square_wide_get(low, product, 0, 128);
    square_mod_add_top(out, low, 128, subtract);
    if (product_width > 128) {
        SquareWide high;
        square_wide_get(high, product, 128, product_width - 128);
        square_apply_f(out, high, product_width - 128, subtract);
    }
}

__device__ __forceinline__ void square_apply_shift_full(
    SquareWide out, const SquareWide product, bool subtract) {
    const int shifts[5] = {0, 4, 6, 10, 32};
    const bool negates[5] = {false, false, true, false, false};
    bool sign = subtract;
#pragma unroll
    for (int i = 0; i < 5; ++i) {
        ...
        if (shift == 0) {
            ...
            SquareWide main, tail;
            square_wide_get(main, product, 0, 256 - shift);
            square_mod_add_top(out, main, shift, sign);
            square_wide_get(tail, product, 256 - shift, shift);
...
__device__ __forceinline__ void square_sub_circuit(
    u64 result[4], const u64 out0[4], const u64 y[4]) {
    SquareWide ywide, ylow, yhigh, ysum;
    SquareWide product_low, product_high, product_sum, out;
    square_wide_from4(ywide, y);
    square_wide_get(ylow, ywide, 0, 128);
    square_wide_get(yhigh, ywide, 128, 128);
    square_wide_add(ysum, ylow, yhigh);
    square_wide_mul(product_low, ylow, ylow);
    square_wide_mul(product_high, yhigh, yhigh);
    square_wide_mul(product_sum, ysum, ysum);
    square_wide_from4(out, out0);

    square_mod_add_top(out, product_low, 0, true);
    square_apply_shift_half(out, product_low, 256, false);
    square_apply_shift_half(out, product_high, 256, false);
    square_apply_shift_full(out, product_high, true);
    square_apply_shift_half(out, product_sum, 258, true);

    square_wide_low4(result, out);
}

// point_add_classical                              (qualified Rust predictor)
__device__ void point_add_classical(u64 gx[4], u64 gy[4], const u64 tx[4], const u64 ty[4],
    ...
    u64 dx[4], dy[4];
    coord_sub_circuit(dx, tx, ox);
    coord_sub_circuit(dy, ty, oy);
    u64 dxr[4], lam[4];
    ...
    fe_add(x, dn, ox3);
    fe_norm(lamn, lam);
    square_sub_circuit(x, x, lamn);
    u64 x2[4], y2[4];
    pingpong_multiply(x2, y2, x, lam, tape);
    coord_sub_circuit(gy, y2, oy);
    coord_rsub_circuit(gx, x2, ox);
}
...
int main(int argc, char **argv) {
    ...
    if (n_ops != EXPECTED_OPS_Q1272_SELECTOR) {
        fprintf(stderr,
            "pingpong_gpu: REFUSING TO RUN -- checkpoint ops %llu do not match "
            "the source-bound Q1272 selector identity %llu.\n",
            (unsigned long long)n_ops,
            (unsigned long long)EXPECTED_OPS_Q1272_SELECTOR);
        return 4;
    }
