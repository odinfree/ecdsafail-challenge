...
// stream at source commit fe0b7bac. The Rust circuit maps each walk round r
// to floor(r * 703 / 697) in the sampled 704-round width schedule, then adds
// one bit at the exact 100 sampled indices in PP_WIDTH_REPAIR_INDEX below.
...
// Exact source fe0b7bac WIDTH_REPAIR membership. A switch keeps the same
// logic available in host and device compilation without a second CUDA
// address-space table. These are sampled-schedule indices, not raw rounds.
...
// Q1274 repair-r100 value_width(): round 0 stays VALUE_WIDTH. Other rounds
// map through the exact Rust width_round_index calculation
// floor(r * 703 / 697), then read the sampled table, apply the sparse +1-bit
// repair, and clamp [8, 259]. Indices >=700 use the floor width 8. The _t
...
PP_HD int pp_value_width_t(int round, const u16* tab) {
    ...
    int schedule_round = round * 703 / 697;
...
PP_HD void pp_square_phase_model(const u64 y[4], const u64 out0[4], u64 out[4],
    ...
    // tri inverse (2). Shift 32's 225-bit add fits the 244-wire ladder.
