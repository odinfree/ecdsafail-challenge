...
static int phasefaultshots(const PP_Prefix* prefix, const u64* comb, u64 nonce) {
    ...
    for (size_t batch = 0; batch < batches; batch++) {
        ...
        // Consume EVERY R/Hmr word in stream order. Relevant source ordinals
        // select the 3,961 modeled predicates; gaps are still squeezed so the
        // next batch begins at the exact Simulator position.
        u64 phase_mask = 0;
        int next_site = 0;
