// ppcpu.cpp — CPU bit-exact reference of the Q1274 repair-r100 ppgpu logic.
...
// Exact hunt-sufficient phase contract. The sibling mirror proved the three
// source-bound boundary families for phase_mask & ~classical_mask. Raw phase
// on already-classical-dirty lanes also contains bare-R vent contributions and
// is intentionally not claimed here. A nonce survives only when BOTH the exact
// classical mask and this complete clean-phase mask are zero.
static int phasefaultshots(const PP_Prefix* prefix, const u64* comb, u64 nonce) {
    PP_PhaseResult result;
    int rc = pp_phase_evaluate(prefix, comb, nonce, &result, "ppcpu");
    if (rc != 0) return rc;
    for (size_t shot : result.clean_phase_shots) printf("%zu\n", shot);
    fprintf(stderr,
            "ppcpu phasefaultshots: nonce=%llu shots=%zu classical=%llu "
            "clean_phase=%zu survivor=%d contract=phase&~classical\n",
            (unsigned long long)nonce, (size_t)PP_NUM_TESTS,
            (unsigned long long)result.classical_faults,
            result.clean_phase_shots.size(),
            result.classical_faults == 0 && result.clean_phase_shots.empty() ? 1 : 0);
    return 0;
}
