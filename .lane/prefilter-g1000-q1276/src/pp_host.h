...
// Hard op-count fingerprint guard: this build serves only the frozen
// Q1276 ladder246/peak1276 stream. The full state digest is asserted by the
// run wrapper.
static const u64 PP_EXPECTED_OPS_G1000 = 12929346ULL;
...
inline void pp_load_prefix(const char* path, PP_Prefix* out) {
    ...
    memcpy(&n, header + 8, 8);
    fclose(f);
    if (n != PP_EXPECTED_OPS_G1000) {
        fprintf(stderr,
                "ppgpu: FATAL: ops stream op count %llu != expected %llu (g1000-q1275); "
                "refusing to run on an unknown stream\n",
                (unsigned long long)n, (unsigned long long)PP_EXPECTED_OPS_G1000);
        exit(2);
    }
    if (n < 96) {
