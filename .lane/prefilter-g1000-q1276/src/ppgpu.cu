...
// model, ROUNDS_DIV=698/ROUNDS_MUL=696) for the frozen Q1276 stream at
// commit 0b6ac181 (g1000 + replay cap 1276 + square ladder 246).
//
// Bit-exactness contract: per-shot fault masks and per-nonce survivor
// verdicts must equal the ppfilter-696 oracle (see FIXTURES.md). All
...
// FNV-1a digest over the checkpoint state + tail + op count, printed on
// stderr so runs can be cross-checked against FIXTURES.md.
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (a == "--ops") ops = next();
        else if (a == "--from") from = strtoull(next(), 0, 10);
    ...
    if (max_faults > PP_NUM_TESTS) {
        fprintf(stderr, "ppgpu: --max-faults exceeds shot count\n");
        return 2;
    }
    ...
    fprintf(stderr, "ppgpu: ops=%s total_ops=%llu state_digest=%016llx\n", ops,
            (unsigned long long)prefix.total_ops, (unsigned long long)state_digest(&prefix));
    ...
    fprintf(stderr,
            ...
            (unsigned long long)hfaults, cnt, (unsigned long long)state_digest(&prefix));
    return 0;
}
