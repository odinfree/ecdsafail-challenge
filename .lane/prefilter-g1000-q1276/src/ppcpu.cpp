...
int main(int argc, char** argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: ppcpu scan FROM COUNT [THREADS] | counts NONCE... | faultshots NONCE | shot NONCE IDX | breakdown NONCE\n");
        return 2;
    }
    ...
    if (mode == "counts") {
        for (int argi = 2; argi < argc; argi++) {
            u64 nonce = strtoull(argv[argi], 0, 10);
            ...
            printf("nonce %llu pred_cls=%llu shots=%zu first=%ld\n",
                   (unsigned long long)nonce, (unsigned long long)pred,
                   shots.size(), first_idx);
        }
        return 0;
    }
