...
`NUMERATOR_RESIDUAL_CARRIER_KILL`

## Current hypothesis

`H12` is terminal KILL under the frozen one-carrier Q cap. The residual is deterministic on the Stage-A corpus and a direct source-cell expression needs no predecessor, but its compute reaches Q1371 and the modular correction reaches Q1544 versus hard cap1370. No semantic row or alternative construction is authorized.

## Current falsifier

H12 failed at the first Stage-B static-Q gate. The exact source-cell compute cumulatively reaches Q1371; the correction owns Q1544. This is `KILL_Q_CAP` before value, phase, or cleanup execution. The measured127-value/4.488941556776-bit entropy does not relax the full256-bit effective-width or exact-source requirement.

## Next action

Stop X010. Preserve `.lane/TEDDY-NUMERATOR-RESIDUAL-CARRIER-KILL.md` and the opt-in fail-closed source; do not attempt a smaller add, lookup, second carrier construction, changed cap, round4, whole-circuit work, optimization, grind, hunt, or provider inside this experiment. Await a separately predeclared structural premise.
...
- X010 Stage-A receipt `.lane/TEDDY-NUMERATOR-RESIDUAL-STAGE-A.md`: complete observation-only 4,096-row census under unchanged corpus/source/event bindings. Raw forward injectivity is64/64 per denominator, finite inverse4096/4096, and raw cleanup exact. Candidate Q1114/ops13,659/emitted T960/executed T894.242; reference Q1546/ops40,605/emitted T4,010/executed T3,944.242. Complete record SHA-256 `7ce27cc97c07d8303c5c0f1f5b86c91c26377a0349225a85425b67a3b62b90b9`; carrier0/correction0. This checkpoint makes no candidate parity or splice claim.
- X010 dependency gate `.lane/TEDDY-NUMERATOR-RESIDUAL-DEPENDENCY.md`: two complete runs have identical normalized record SHA-256 `7ce27cc97...` and semantic analysis SHA-256 `a1d394d6...`. Residual has127 values/entropy4.488941556776/information lower bound7 but full256-bit support and variation. Constant, sign-linear, affine-XOR, and simplified arithmetic are KILL; the latter misses116 rows by exactly +/-2^53. `(sign1,sign2,c1)` has no conflicting fiber. Exactly one direct source-cell compute/correct/inverse construction was selected; its terminal static-Q result follows below.
- X010 terminal `.lane/TEDDY-NUMERATOR-RESIDUAL-CARRIER-KILL.md`: selected one-carrier source composition compiles, then fails before simulation. Base Q1284; inherited boundary Q1370; compute Q1371 is the first overage; `mod_add_qq_lowq` correction owns Q1544. Static shape43,584 ops/emitted T4,646, with carrier stages819/2,048/819 and total debt+29,925 ops/+3,686 T. Two exact KILL receipts match at SHA-256 `148513dc...`. Semantic/relative-phase/cleanup rows0; no splice or second construction authorized.
