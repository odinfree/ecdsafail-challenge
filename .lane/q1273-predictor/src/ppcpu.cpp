...
// oracle proves these measured-boundary predicates only for
// phase_mask & ~classical_mask. Raw phase on already-classically-dirty shots
// is intentionally not claimed. A survivor requires both exact masks to be
// zero.
static int phasefaultshots(const PP_Prefix* prefix, const u64* comb, u64 nonce) {
    ...
    for (size_t batch = 0; batch < batches; batch++) {
        ...
        // Consume every R/Hmr word in exact stream order. Relevant ordinals
        // select the modeled predicates; gaps are still squeezed so every
        // batch begins at the same simulator position.
        u64 phase_mask = 0;
        int next_site = 0;
