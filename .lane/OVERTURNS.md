...
| O-011 | Moving overflow erasure before high propagation is phase-safe if its measured value temporarily restores the live ladder control. | E-013 keeps all values exact and dirty zero, but the frozen phase mask remains `0x24`. | Run the inherited erasure at the apex, restore overflow from its bit, then complete the same coherent ladder reverse. | overturned; measurement cannot cross live ladder |
| O-012 | Removing measurement venting from the high200 carry path is sufficient to clear E-011 phase. | E-014's high path contains no HMR/R, costs only four additional emitted T, and remains value/ancilla exact, but phase is still `0x30`. | Replace the vented controlled increment with its complete coherent inverse under the same live carry. | overturned; micro-repair family closed |
